...
static ggml_type kvarn_fallback_cache_type(int32_t bits) {
    // Den's ggml build exposes only the 4/5/8-bit block types below; the
    // remaining bit depths fall back to the nearest available standard type
    // for layers that are not eligible for structured KVarN storage.
    switch (bits) {
        case 2:
        case 3:
...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        ...
        string_format(
            "KV cache data type for K (sliding-window layers)\n"
            "allowed values: %s\n"
            "(default: %s)",
            get_all_kv_cache_types(true).c_str(),
            ggml_type_name(params.cache_type_k)
    ...
    add_opt(common_arg(
        ...
        string_format(
            "KV cache data type for V (sliding-window layers)\n"
            "allowed values: %s\n"
            "(default: %s)",
            get_all_kv_cache_types(true).c_str(),
            ggml_type_name(params.cache_type_v)
    ...
    add_opt(common_arg(
        ...
        "offload all the expert weights to the device instead of only the active experts (default: offload only the active experts)",
        [](common_params & params) {
    ...
    // adaptive draft-max controller (profit EWMA) -- ported from beellama
    add_opt(common_arg(
        {"--spec-dm-controller"}, "MODE",
        string_format("adaptive draft-max controller: off or profit (default: %s)",
    ...
    add_opt(common_arg(
        ...
        string_format("minimum samples before a depth's profit estimate is trusted (default: %d)",
    ...
    add_opt(common_arg(
        ...
        string_format("samples to collect before acting on any depth (default: %d)",
    ...
    add_opt(common_arg(
        ...
        string_format("cycles between no-spec baseline re-probes (default: %d)",
