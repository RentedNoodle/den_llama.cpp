...
bool gpt_params_find_arg(int argc, char ** argv, const std::string & arg, gpt_params & params, int & i, bool & invalid_param) {
    ...
    if (arg == "--expert-cache-mb") {
        CHECK_ARG
        int32_t mb = std::stoi(argv[i]);
        if (mb < 0) {
            fprintf(stderr, "error: Invalid value for --expert-cache-mb: %d (must be >= 0)\n", mb);
            invalid_param = true;
            return true;
        }
        params.expert_cache_mb = mb;
        return true;
    }
    if (arg == "--cpu-moe-prefetch") {
        CHECK_ARG
        int32_t depth = std::stoi(argv[i]);
        if (depth < 0) {
            fprintf(stderr, "error: Invalid value for --cpu-moe-prefetch: %d (must be >= 0)\n", depth);
            invalid_param = true;
            return true;
        }
        params.cpu_moe_prefetch = depth;
        return true;
    }
    if (arg == "--fit") {
        params.fit = true;
...
struct llama_context_params common_context_params_to_llama(const gpt_params & params) {
    ...
    cparams.expert_prefetch   = params.cpu_moe_prefetch;
