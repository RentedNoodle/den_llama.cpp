...
static void common_escha_logits_probe(const llama_context * ctx, int idx, const common_sampler & sampler, llama_token backend_id) {
    static std::atomic<bool> emitted { false };
    const char * const env = std::getenv("DEN_ESCHA_LOGITS_PROBE");
    if (env == nullptr || env[0] == '\0' || env[0] == '0' || ctx == nullptr || emitted.exchange(true)) {
        return;
    }

    llama_token top_id = LLAMA_TOKEN_NULL;
    float top_logit = -INFINITY;
    for (size_t i = 0; i < sampler.cur_p.size; ++i) {
        if (sampler.cur_p.data[i].logit > top_logit) {
            top_logit = sampler.cur_p.data[i].logit;
            top_id = sampler.cur_p.data[i].id;
        }
    }

    auto * ctx_mut = const_cast<llama_context *>(ctx);
    const bool has_logits = llama_get_sampled_logits_ith(ctx_mut, idx) != nullptr;
    const bool has_probs = llama_get_sampled_probs_ith(ctx_mut, idx) != nullptr;
    const bool backend_buffers = has_logits || has_probs || backend_id != LLAMA_TOKEN_NULL;
    LOG_INF("%s: candidate_top=%d:%g candidates=%zu candidate_set=%s sampled_logits=%s sampled_probs=%s backend_buffers=%s backend_token=%d\n",
            __func__, top_id, top_logit, sampler.cur_p.size,
            sampler.cur_p.size == (size_t) llama_vocab_n_tokens(llama_model_get_vocab(llama_get_model(ctx))) ? "full" : "reduced",
            has_logits ? "yes" : "no", has_probs ? "yes" : "no",
            backend_buffers ? "yes" : "no", backend_id);
}
...
struct common_sampler * common_sampler_init(
    ...
    // A2.1: the grammar sampler joins the chain (before the top-k) so its
    // full-vocab mask runs in the backend (GPU) sampling path; the chain's
    // apply/backend_apply then handle the constraint in both modes.
    if (grmr) {
        samplers.push_back(grmr);
    }
    ...
    // A2.1: the grammar sampler now has a backend path (the full-vocab allowed
    // mask applied to the logits in-graph) — structured output can run with -bs.
    // The grammar's mask is computed host-side per position and overlapped with
    // the decode; the top-k/softmax/sample stay on the GPU.
...
llama_token common_sampler_sample(struct common_sampler * gsmpl, struct llama_context * ctx, int idx, bool grammar_first) {
    ...
    common_escha_logits_probe(ctx, idx, *gsmpl, id);
    ...
    // return that token id directly.
    {
        if (id != LLAMA_TOKEN_NULL) {
            ...
            // A2.1: the grammar's mask runs in the backend graph now (the grmr is
            // part of the chain), so a backend-selected token is grammar-valid.
...
std::vector<llama_token> common_sampler_sample_and_accept_n(struct common_sampler * gsmpl, struct llama_context * ctx, const llama_tokens & draft, const std::vector<common_speculative_token_dist> & dists, bool grammar_first) {
