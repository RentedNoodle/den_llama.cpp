    ...
    def get_total_parameter_count(self) -> tuple[int, int, int, int]:
        ...
        for tensors in self.tensors:
            for name, info in tensors.items():
                ...
                if "_exps." in name and not name.endswith((".scale", ".input_scale")):
                    # NVFP4 `.scale` companions are 1-D per-expert metadata, not weights —
                    # exclude them from expert-param accounting (they'd break shape[-3]).
