...
###### NVFP4 (DenQuant E2M1 + UE4M3) quantization ######
# On-disk NVFP4 GGUF blocks are 144 bytes per 256 elements: 4 sub-blocks of
# [4x UE4M3 scale bytes][32x E2M1 nibble bytes]. The den_llama.cpp loader
# de-interleaves them into a 160-byte in-memory block and folds the per-tensor /
# per-expert global scale (companion `.scale` F32 tensor) into bytes 152:155.
# Dequant: value = ue4m3(scale) * global * std_e2m1[nibble]   (global == scale_2)


def _ue4m3_decode(codes: np.ndarray) -> np.ndarray:
    """Decode UE4M3 scale bytes to float32 (mirrors nvfp4_ue4m3_to_fp32)."""
    codes = np.asarray(codes, dtype=np.int32) & 0xFF
    exp = (codes >> 3) & 0x0F
    man = codes & 0x07
    out = np.zeros(codes.shape, dtype=np.float32)
    normal = exp > 0
    out[normal] = (1.0 + man[normal] / 8.0) * (2.0 ** (exp[normal] - 7))
    sub = exp == 0
    out[sub] = (man[sub] / 8.0) * (2.0 ** -7)
    out[codes >= 0x7F] = 0.0
    return out


def _ue4m3_encode(vals: np.ndarray) -> np.ndarray:
    """Encode positive float32 values to UE4M3 scale bytes, clamped [2^-9, 448]."""
    vals = np.asarray(vals, dtype=np.float32)
    code = np.zeros(vals.shape, dtype=np.int32)
    nz = np.isfinite(vals) & (vals > 0)
    if not np.any(nz):
        return code
    vv = np.where(nz, vals, 0.0)
    vv = np.clip(vv, 2.0 ** -9, 448.0)
    e = np.floor(np.log2(vv)).astype(np.int32)
    sub = vv < (2.0 ** -6)  # subnormal or inside the subnormal/normal gap
    f = vv / (2.0 ** e) - 1.0
    mant = np.round(f * 8.0).astype(np.int32)
    expf = e + 7
    over = mant >= 8
    mant = np.where(over, 0, mant)
    expf = expf + over.astype(np.int32)
    c = (expf << 3) | mant
    sm = np.clip(np.round(vv / (2.0 ** -10)).astype(np.int32), 1, 7)
    c = np.where(sub, sm, c)
    c = np.clip(c, 0x02, 0x7E)
    return np.where(nz, c, 0)


def _bf16_to_f32(u16: np.ndarray) -> np.ndarray:
    """Decode raw BF16 bit patterns (uint16) to float32."""
    u = np.asarray(u16, dtype=np.int32) & 0xFFFF
    sign = (u >> 15) & 1
    exp = (u >> 7) & 0xFF
    man = u & 0x7F
...
def _e2m1_code(mag: np.ndarray) -> np.ndarray:
    """E2M1 magnitude code (0..7) with half-to-even rounding at the odd bounds."""
    q = 2.0 * np.clip(mag, 0.0, 6.0)
    edges = np.array([0.0, 0.5, 1.5, 2.5, 3.5, 5.0, 7.0, 10.0, 12.0], dtype=np.float32)
    code = np.searchsorted(edges, q, side="right").astype(np.int32) - 1
    code -= ((q == 0.5) | (q == 2.5) | (q == 5.0) | (q == 10.0)).astype(np.int32)
    return np.clip(code, 0, 7)


def _quantize_nvfp4_2d(x: np.ndarray, chunk_rows: int = 512) -> tuple[np.ndarray, np.float32]:
    """Quantize a 2D f32 [rows, K] tensor to NVFP4 blocks.

    Returns (packed_uint8 [rows, (K//256)*144], global_scale float32).
    The global scale is emitted as a `.scale` companion (folded to bytes 152:155
    at load) so it is NOT stored inside the 144-byte blocks.
    """
    # LazyTorchTensor/LazyNumpyTensor must be materialized first:
    # np.ascontiguousarray(lazy) returns the zeroed strided meta, not the data.
    if "Lazy" in type(x).__name__:
        from gguf.lazy import LazyNumpyTensor
        x = LazyNumpyTensor.to_eager(x)
    x = np.asarray(x)
    if x.dtype == np.uint16:  # BF16 bit patterns (uint16)
        x = _bf16_to_f32(x)
    ...
    N, K = x.shape
    assert K % 256 == 0, f"NVFP4 requires K % 256 == 0, got K={K}"
    nb = K // 256

    gamax = 0.0
    for r0 in range(0, N, chunk_rows):
        r1 = min(r0 + chunk_rows, N)
        bamax = np.abs(x[r0:r1].reshape(-1, 16, 16)).max(axis=-1).max()
        gamax = max(gamax, float(bamax))

    if gamax <= 0.0:
        return np.zeros((N, nb * 144), dtype=np.uint8), np.float32(0.0)

    norm = np.float32(gamax / (6.0 * 448.0))
    out = np.empty((N, nb * 144), dtype=np.uint8)
    scale_inv = 448.0 / gamax

    for r0 in range(0, N, chunk_rows):
        r1 = min(r0 + chunk_rows, N)
        blk = x[r0:r1].reshape(-1, 256)
        nb_c = blk.shape[0]
        groups = blk.reshape(nb_c, 16, 16)
        bamax = np.abs(groups).max(axis=-1)              # (nb_c, 16)
        codes = _ue4m3_encode(bamax * scale_inv).astype(np.uint8)
        scale_eff = _ue4m3_decode(codes) * norm          # (nb_c, 16)
        q = np.abs(blk.reshape(nb_c, 16, 16)) / scale_eff[:, :, None]
        q = np.where(scale_eff[:, :, None] > 0.0, q, 0.0)
        mag = _e2m1_code(q)                              # (nb_c,16,16)
        sign = (blk < 0).reshape(nb_c, 16, 16).astype(np.int32)
        nib = ((sign << 3) | mag).astype(np.uint8)       # (nb_c,16,16)
        lo = nib[:, :, :8]
        hi = nib[:, :, 8:]
        nb_bytes = ((hi.astype(np.uint16) << 4) | lo).astype(np.uint8).reshape(nb_c, 128)
        packed = np.empty((nb_c, 144), dtype=np.uint8)
        for sb in range(4):
            packed[:, sb * 36:sb * 36 + 4] = codes[:, 4 * sb:4 * sb + 4]
            packed[:, sb * 36 + 4:sb * 36 + 36] = nb_bytes[:, 32 * sb:32 * sb + 32]
        out[r0:r1] = packed.reshape(r1 - r0, nb * 144)

    return out, norm


def _quantize_nvfp4(data: np.ndarray, per_expert: int) -> tuple[np.ndarray, np.ndarray]:
    """Quantize f32 weights to NVFP4.

    per_expert == 1 -> dense tensor: returns packed (rows, nb*144) + scale[1].
    per_expert > 1  -> fused MoE tensor [n_experts, N, K]: returns packed
    (n_experts, N, nb*144) + one global scale per expert.
    """
    # Materialize lazy tensors FIRST — np.ascontiguousarray(lazy) yields the
    # zeroed strided meta, so any ascontiguousarray before this point loses data.
    if "Lazy" in type(data).__name__:
        from gguf.lazy import LazyNumpyTensor
        data = LazyNumpyTensor.to_eager(data)
    if per_expert > 1:
        n_exp, N, K = data.shape
        packed = np.empty((n_exp, N, (K // 256) * 144), dtype=np.uint8)
        scales = np.empty(n_exp, dtype=np.float32)
        for e in range(n_exp):
            p, s = _quantize_nvfp4_2d(np.ascontiguousarray(data[e]))
            packed[e] = p
            scales[e] = s
        return packed, scales
    p, s = _quantize_nvfp4_2d(np.ascontiguousarray(data))
    return p, np.asarray([s], dtype=np.float32)
...
class Model:
    ...
    def map_tensor_name(self, name: str, try_suffixes: Sequence[str] = (".weight", ".bias")) -> str:
        # HF names the LM head "lm_head.weight"; llama.cpp archs expect "output.weight"
        # (LLM_TENSOR_OUTPUT). The gguf-py QWEN35/QWEN35MOE maps lack the alias, so the
        # raw lm_head.weight was written and the loader's done_getting_tensors check failed
        # (expected 427, got 426). Handle at the base so every arch is covered.
    ...
    def quantize_nvfp4(self, data: np.ndarray, new_name: str) -> tuple[np.ndarray, np.ndarray]:
        """NVFP4-quantize `data`; return (packed bytes, `.scale` companion values)."""
        del new_name
        if data.ndim == 3:
            # Fused MoE expert tensor [n_experts, N, K]: one global scale per expert.
            # (Config keys vary: num_local_experts / moe_num_experts / num_experts —
            #  the first dim IS the expert count, so use it directly.)
            return _quantize_nvfp4(data, per_expert=data.shape[0])
        # Dense (or shared-expert) tensor: single global scale.
        return _quantize_nvfp4(data, per_expert=1)
            ...
            for new_name, data in ((n, d.squeeze().numpy()) for n, d in self.modify_tensors(data_torch, name, bid)):
                ...
                if data_qtype is False and any(
                    ...
                    if self.ftype == gguf.LlamaFileType.MOSTLY_NVFP4:
                        # embeddings stay unquantized (BF16); output/attn -> NVFP4
                ...
                nvfp4_scale: np.ndarray | None = None
                if data_qtype == gguf.GGMLQuantizationType.NVFP4:
                    # Custom NVFP4 quantizer (E2M1 nibbles + UE4M3 block scales +
                    # per-tensor/per-expert global `.scale` companion). Not a
                    # ggml __Quant, so handled here rather than in quants.py.
                    data, nvfp4_scale = self.quantize_nvfp4(data, new_name)
                ...
                # NVFP4 companion: per-tensor/per-expert global scale, folded by
                # the loader into NULLGLASS header bytes 152:155 at load time.
                if nvfp4_scale is not None and new_name.endswith(".weight"):
                    self.gguf_writer.add_tensor(
                        new_name[:-len(".weight")] + ".scale", nvfp4_scale.astype(np.float32)
                    )
...
def _reorder_v_heads(tensor: Tensor, dim: int, num_k_heads: int, num_v_per_k: int, head_dim: int) -> Tensor:
    ...
    Ported from upstream llama.cpp `_LinearAttentionVReorderBase`. Qwen3.5 linear
    attention may have num_key_heads < num_value_heads. HF stores V rows grouped by
    K head: [G0_v0..v{r-1}, G1_v0..v{r-1}, ...]; ggml binary ops use tiled broadcast
    [K0, K1, ..., K0, K1, ...]. We reorder V heads to tiled order.
    """
...
class Qwen3_5MoeModel(Qwen2MoeModel):
    ...
    def map_tensor_name(self, name: str, try_suffixes = (".weight", ".bias")) -> str:
        # HF names the LM head "lm_head.weight"; llama.cpp archs expect "output.weight"
        # (LLM_TENSOR_OUTPUT). The gguf-py QWEN35 map lacks the alias -> raw lm_head.weight
        # was written and the qwen35moe loader ignored it (done_getting_tensors mismatch).
    ...
    def set_gguf_parameters(self):
        ...
        # Qwen3.5 always applies interleaved MRoPE; write rope.dimension_sections even
        # when rope_parameters omits mrope_section (loader treats it as required).
        if not isinstance(rope_params, dict) or not rope_params.get("mrope_section"):
            self.gguf_writer.add_rope_dimension_sections([11, 11, 10, 0])
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        # NOTE: np.asarray(lazy.numpy()) returns the zeroed strided meta, not the data;
        # LazyNumpyTensor.to_eager() is the correct materializer.
        if 'Lazy' in type(data_torch).__name__:
            from gguf.lazy import LazyNumpyTensor
            data_torch = torch.from_numpy(LazyNumpyTensor.to_eager(data_torch.numpy()))
        ...
        # ── Separate per-expert tensors → fused 3D ffn_gate_exps/ffn_up_exps/ffn_down_exps ──
        # Ornith-style checkpoints ship per-expert weights
        #   model.language_model.layers.{i}.mlp.experts.{n}.{gate,up,down}_proj.weight
        # stripped above to `layers.{i}.mlp.experts.{n}.{gate,up,down}_proj.weight`.
        # Mirror the base Qwen2MoeModel merge: accumulate the per-expert 2D tensors
        # and torch.stack them into 3D fused tensors once a full block has arrived.
        # Yield in PyTorch orientation [n_experts, ffn, hidden] — NO pre-permute, the
        # gguf writer reverses dims in the header (base Qwen2MoeModel is correct).
        if re.search(r"mlp\.experts\.\d+\.(gate|up|down)_proj\.weight", name):
            n_experts = self.hparams.get("num_experts") or self.hparams.get("num_local_experts")
            if n_experts is None:
                raise ValueError("Qwen3_5MoeModel: cannot merge per-expert tensors — "
                                 "num_experts/num_local_experts missing from hparams")
            assert bid is not None
            if self._experts is None:
                self._experts = [{} for _ in range(self.block_count)]
            self._experts[bid][name] = data_torch
            if len(self._experts[bid]) >= n_experts * 3:
                # Qwen3_5MoeModel.modify_tensors is a GENERATOR (other branches yield),
                # so a bare `return <list>` would silently discard the fused tensors.
                # Yield each fused 3D tensor in PyTorch orientation instead.
                for w_name, gguf_name in (("down_proj", "ffn_down_exps"),
                                          ("gate_proj", "ffn_gate_exps"),
                                          ("up_proj", "ffn_up_exps")):
                    datas: list[Tensor] = []
                    for xid in range(n_experts):
                        ename = f"layers.{bid}.mlp.experts.{xid}.{w_name}.weight"
                        datas.append(self._experts[bid][ename])
                        del self._experts[bid][ename]
                    data_torch = torch.stack(datas, dim=0)
                    yield (f"blk.{bid}.{gguf_name}.weight", data_torch)
                return
            return

        # ── V-head reorder (ported from upstream _LinearAttentionVReorderBase) ──
        ...
        if num_k_heads > 0 and num_v_heads > 0 and num_k_heads != num_v_heads and "linear_attn." in name:
            ...
            if ".in_proj_qkv." in name:
                ...
                v = _reorder_v_heads(v, 0, num_k_heads, num_v_per_k, head_v_dim)
                data_torch = torch.cat([q, k, v], dim=0)
            elif ".in_proj_z." in name:
                data_torch = _reorder_v_heads(data_torch, 0, num_k_heads, num_v_per_k, head_v_dim)
            elif ".in_proj_b." in name or ".in_proj_a." in name:
                data_torch = _reorder_v_heads(data_torch, 0, num_k_heads, num_v_per_k, 1)
            elif ".A_log" in name or ".dt_bias" in name or ".dt_proj" in name:
                if data_torch.ndim == 1:
                    data_torch = _reorder_v_heads(
                        data_torch.unsqueeze(-1), 0, num_k_heads, num_v_per_k, 1
                    ).squeeze(-1)
                else:
                    data_torch = _reorder_v_heads(data_torch, -1, num_k_heads, num_v_per_k, 1)
            elif ".conv1d" in name:
                data = data_torch.squeeze()
                qk_channels = head_k_dim * num_k_heads * 2
                qk_part = data[:qk_channels]
                v_part = data[qk_channels:]
                v_part = _reorder_v_heads(v_part, 0, num_k_heads, num_v_per_k, head_v_dim)
                data_torch = torch.cat([qk_part, v_part], dim=0)
            elif ".out_proj." in name:
                data_torch = _reorder_v_heads(data_torch, 1, num_k_heads, num_v_per_k, head_v_dim)
        elif "conv1d" in name:
            # Conv1d kernel is [channels, 1, kernel] -> squeeze to 2-D (upstream Qwen3NextModel)
            data_torch = data_torch.squeeze()

        # ── Fused expert tensors: gate_up_proj [256,1024,2048] → gate [256,512,2048] + up [256,512,2048] ──
        if "experts.gate_up_proj" in name:
            ...
            # Yield in PyTorch orientation [n_experts, ffn_dim, hidden] - no permute:
            # the gguf writer reverses dims in the header, so a pre-permute would
            # double-reverse and write transposed weights (base Qwen2MoeModel is correct).
            import numpy as np
            gate_np = data_torch[:, :gate_dim, :].contiguous().numpy()
            up_np   = data_torch[:, gate_dim:, :].contiguous().numpy()
        ...
        # ── Expert down_proj: [256, 2048, 512] keep PyTorch orientation (no permute) ──
        if "experts.down_proj" in name:
            import numpy as np
            data_np = data_torch.contiguous().numpy().copy()
        ...
        # ── Router gate: [256, 2048] — keep PyTorch orientation (no transpose) ──
        ...
        # Special case: linear_attn.norm → ssm_norm (must be handled BEFORE stripping "linear_attn.")
        ...
        # LM head: HF "lm_head.weight" -> llama.cpp "output.weight" (LLM_TENSOR_OUTPUT).
        # Without this the qwen35moe loader ignores it -> done_getting_tensors mismatch.
        if name == "lm_head.weight":
            name = "output.weight"
        yield (name, data_torch)
...
class Qwen3_5Model(Qwen2Model):
    ...
    def map_tensor_name(self, name: str, try_suffixes = (".weight", ".bias")) -> str:
        # HF names the LM head "lm_head.weight"; llama.cpp archs expect "output.weight"
        # (LLM_TENSOR_OUTPUT). The gguf-py QWEN35 map lacks the alias -> raw lm_head.weight
        # was written and the qwen35 loader ignored it (done_getting_tensors mismatch).
        if name == "lm_head.weight":
            return "output.weight"
        return super().map_tensor_name(name, try_suffixes)
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        # LM head: HF "lm_head.weight" -> llama.cpp "output.weight" (LLM_TENSOR_OUTPUT).
        # Without this the qwen35 loader ignores it -> done_getting_tensors mismatch.
        if name == "lm_head.weight":
            name = "output.weight"
        return [(name, data_torch)]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--outtype", type=str, choices=["f32", "f16", "bf16", "q8_0", "q4_0", "q4_1", "q5_0", "q5_1", "q6_0", "nvfp4", "auto"], default="f16",
