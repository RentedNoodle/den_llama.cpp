...
std::pair<ggml_tensor *, ggml_tensor *> delta_net::build_fused_delta_net(ggml_context * ctx0,
    ...
    // q/k need cont too: the kernel reads them with flat offsets [D, T, Hk]
    // which only matches a contiguous layout. Without cont, multi-token graphs
    // (prompt eval, MTP draft) read q/k across wrong tokens/heads.
    q    = ggml_cont(ctx0, q);
    k    = ggml_cont(ctx0, k);
...
std::pair<ggml_tensor *, ggml_tensor *> delta_net::build_beta_gate(llama_context & lctx, ggml_context * ctx0,
    ...
    // GGUF stores ssm_a = -exp(A_log) (see convert_hf_to_gguf.py Qwen3_5Model).
    // exp(A_log) = -ssm_a. gate = exp(A_log) * softplus = -ssm_a * softplus (POSITIVE),
    // kernel computes decay = exp(-gate). Matches HF decay exactly.
    ggml_tensor * a_neg = ggml_neg(ctx0, ssm_a);
    cb(a_neg, "A_neg", il);
    ggml_tensor * gate = ggml_mul(ctx0, alpha_softplus, a_neg);
    cb(gate, "gate", il);
...
ggml_tensor * delta_net::build_qkv(ggml_context * ctx0, ggml_tensor * state_storage, ggml_tensor * ssm_conv1d,
    ...
    // Write back conv + SSM state via ggml_concat_inplace (matches ik_llama.cpp).
    // concat_inplace has no CUDA backend (runs on CPU) - for GPU offload
    // this path must be revisited, but CPU decode correctness is priority.
    ...
    ggml_tensor * state_cpy = ggml_concat_inplace(ctx0, new_conv_flat, new_ssm_flat, state_dst, 0);
