...
bool llama_context::can_reuse_graph(const llama_batch & u_batch) {
    if (getenv("DEN_DUMP_HIDDEN") != nullptr) return false; // fresh buffers for reliable dump reads
        ...
        // (converter emits 1D float32 arrays of per-tile norm factors)
        {
            ...
            for (const auto & w : ml.weights) {
                ...
                if (name_len > 2 && name[name_len-2] == '_' && name[name_len-1] == 'n') {
                    ...
                    LLAMA_LOG_INFO("NVFP4 norm: %s = %.9g (n=%zu)\n", base_name.c_str(), vec.empty()?0.0f:vec[0], vec.size());
                }
                // Upstream converters emit ".scale"/".input_scale" companion F32
                // tensors holding the per-tensor GLOBAL scale (ModelOpt format).
                // The OMMA kernel multiplies per-tile by tile_norms[] — fold the
                // global scale into that channel so upstream NVFP4 GGUFs work.
                static const char * const gsuffixes[] = { ".scale" };  // weight-global ONLY; .input_scale is activation scale
                for (const char * suf : gsuffixes) {
                    const size_t slen = strlen(suf);
                    if (name_len > slen && strncmp(name + name_len - slen, suf, slen) == 0) {
                        const size_t n_floats = (size_t)ggml_nelements(w.tensor);
                        // KEY MUST MATCH src0->name used by den_get_nvfp4_norm_array().
                        // The weight tensor is "<base>.weight" (e.g. blk.0.attn_qkv.weight),
                        // and the companion is "<base>.scale"/"<base>.input_scale".
                        // Build the key as the weight tensor's actual name.
                        std::string base_name = std::string(name, name_len - slen) + ".weight";
                        if (n_floats >= 1) {
                            std::vector<float> tmp(n_floats);
                            if (ml.use_mmap) {
                                const auto & mapping = ml.mappings.at(w.idx);
                                memcpy(tmp.data(), (uint8_t *)mapping->addr() + w.offs, n_floats * sizeof(float));
                            } else {
                                const auto & file = ml.files.at(w.idx);
                                file->seek(w.offs, SEEK_SET);
                                file->read_raw(tmp.data(), n_floats * sizeof(float));
                            }
                            // broadcast scalar global scale over per-tile array
                            auto & vec = model.nvfp4_norm_factors[base_name];
                            if (vec.empty()) {
                                vec.assign(n_floats, tmp[0]);
                            } else {
                                for (auto & v : vec) v *= tmp[0];
                            }
                            LLAMA_LOG_INFO("NVFP4 norm: %s = %.9g (n=%zu)\n", base_name.c_str(), vec.empty()?0.0f:vec[0], vec.size());
                        }
                        break;
...
// Per-node LAST-token fingerprint for GPU-vs-CPU divergence localization.
// Scans the actual graph node names produced by the cb callback
// ("<name>-<il>") and prints rms/l1/min/max/first-4 for the last token.
static void den_dump_nodes(const ggml_cgraph * gf, int n_embd) {
    GGML_UNUSED(n_embd);
    for (int i = 0; i < gf->n_nodes; ++i) {
        const ggml_tensor * t = gf->nodes[i];
        if (t->type != GGML_TYPE_F32 || t->ne[1] < 1) {
            continue;
        }
        const char * nm = t->name;
        bool want = false;
        if (strncmp(nm, "l_out-", 6) == 0 || strncmp(nm, "fa-", 3) == 0 ||
            strncmp(nm, "attn_out_gated-", 15) == 0 || strncmp(nm, "attn_out-", 9) == 0 ||
            strncmp(nm, "result_output", 13) == 0 || strncmp(nm, "attn_norm-", 10) == 0 ||
            strncmp(nm, "Qcur_roped-", 11) == 0 || strncmp(nm, "Kcur_roped-", 11) == 0) {
            want = true;
        }
        if (!want) {
            continue;
        ...
        std::vector<float> last((size_t) n0);
        ggml_backend_tensor_get(t, last.data(), (size_t)(t->ne[1]-1)*n0*sizeof(float), (size_t)n0*sizeof(float));
        double ss = 0.0; double l1 = 0.0; float mn = 1e30f; float mx = -1e30f;
        for (int64_t k = 0; k < n0; ++k) {
            const float v = last[(size_t)k];
            ss += (double)v*v; l1 += (double)fabsf(v);
            mn = fminf(mn, v); mx = fmaxf(mx, v);
        }
        fprintf(stderr, "DENNODE %-16s ne0=%lld ne1=%lld rms=%.6f l1=%.3f min=%.4f max=%.4f fp=[%.4f %.4f %.4f %.4f]\n",
                nm, (long long)n0, (long long)t->ne[1], sqrt(ss/(double)n0), l1, mn, mx,
                last[0], last[1], last[2], last[3]);
    }
}

// ── Clean per-node observer via ggml_backend_sched eval callback ─────────
// gated by DEN_EVAL_DUMP. Unlike the DEN_DUMP_HIDDEN graph instrumentation
// (which force-outputs ~160 intermediate tensors and corrupts the allocator,
// producing exactly the "GPU garbles / CPU coherent" symptom), this callback
// observes each node DURING compute, before its buffer is freed. No graph
// nodes are added, so the computation is bit-identical to a plain run.
static bool den_node_wanted(const char * name) {
    return strncmp(name, "l_out-", 6) == 0 ||
           strncmp(name, "fa-", 3) == 0 ||
           strncmp(name, "attn_norm-", 10) == 0 ||
           strncmp(name, "attn_out_gated-", 15) == 0 ||
           strncmp(name, "Qcur_roped-", 11) == 0 ||
           strncmp(name, "Kcur_roped-", 11) == 0 ||
           strncmp(name, "q_in", 4) == 0 || strncmp(name, "k_in", 4) == 0 ||
           strncmp(name, "v_in", 4) == 0 || strncmp(name, "g_in", 4) == 0 ||
           strncmp(name, "beta_in", 7) == 0 ||
           strncmp(name, "q_fused", 7) == 0 || strncmp(name, "k_fused", 7) == 0 ||
           strncmp(name, "v_fused", 7) == 0 || strncmp(name, "g_fused", 7) == 0 ||
           strncmp(name, "delta_net_fused_raw", 19) == 0 ||
           strncmp(name, "output_tokens", 13) == 0 ||
           strncmp(name, "ffn_moe_weighted", 16) == 0 ||
           strncmp(name, "ffn_moe_down", 12) == 0 ||
           strncmp(name, "ffn_moe_logits", 14) == 0 || strncmp(name, "ffn_inp_normed", 14) == 0 ||
           strncmp(name, "ffn_shexp_out", 13) == 0 ||
           strncmp(name, "state_in", 8) == 0 || strncmp(name, "state_predelta", 14) == 0 ||
           strncmp(name, "qkv_mixed", 9) == 0 || strncmp(name, "linear_attn_qkv_mixed", 21) == 0 ||
           strncmp(name, "linear_attn_mixed_qkvz", 22) == 0 ||
           strncmp(name, "conv_output_raw", 15) == 0 || strncmp(name, "conv_output_silu", 16) == 0 ||
           strncmp(name, "q_conv", 6) == 0 || strncmp(name, "k_conv", 6) == 0 || strncmp(name, "v_conv", 6) == 0 ||
           strncmp(name, "attn_output", 11) == 0 || strncmp(name, "routed_out", 10) == 0 ||
           strncmp(name, "ffn_out", 7) == 0 ||
           strcmp(name, "result_output") == 0;
}

static bool den_eval_cb(struct ggml_tensor * t, bool ask, void * user_data) {
    GGML_UNUSED(user_data);
    if (ask) {
        return den_node_wanted(t->name); // isolate only the nodes we inspect
    }
    // post-compute: split backend is synchronized, data is readable
    if (den_node_wanted(t->name) && t->type == GGML_TYPE_F32) {
        const int64_t n = ggml_nelements(t);
        if (n > 0 && n <= 4*1024*1024) {
            std::vector<float> buf((size_t) n);
            ggml_backend_tensor_get(t, buf.data(), 0, (size_t)n*sizeof(float));
            double ss = 0.0; double l1 = 0.0; float mn = 1e30f; float mx = -1e30f; uint32_t hsh = 2166136261u;
            for (int64_t i = 0; i < n; ++i) {
                const float v = buf[(size_t) i];
                ss += (double)v*v; l1 += (double)fabsf(v);
                mn = fminf(mn, v); mx = fmaxf(mx, v);
                uint32_t bits; memcpy(&bits, &v, 4);
                hsh = (hsh ^ bits) * 16777619u;
            }
            fprintf(stderr, "EVAL %-16s n=%lld rms=%.6f l1=%.3f min=%.4f max=%.4f h=%08x fp=[%.4f %.4f %.4f %.4f]\n",
                    t->name, (long long)n, sqrt(ss/(double)n), l1, mn, mx, hsh, buf[0], buf[1], buf[2], buf[3]);
            if (getenv("DEN_FULLDUMP") && (strncmp(t->name,"ffn_moe_logits",14)==0 || strncmp(t->name,"ffn_moe_down",12)==0 || strncmp(t->name,"ffn_inp_normed",14)==0 || strncmp(t->name,"attn_output",11)==0 || strncmp(t->name,"delta_net_fused_raw",19)==0)) {
                const char * od = getenv("DEN_FULLDUMP");
                FILE * fh = fopen(od, "a");
                if (fh) {
                    fprintf(fh, "FULL %s n=%lld\n", t->name, (long long)n);
                    for (int64_t i = 0; i < n; i++) {
                        fprintf(fh, "%.9g ", buf[(size_t)i]);
...
// Per-layer hidden-state RMS for the LAST token — Atlas-style divergence comparator.
// Called after every llama_graph_compute in llama_decode_internal so both the
// standard and recurrent (GDN/SSM) decode paths dump.
static void den_dump_hidden(const ggml_cgraph * gf, int n_embd) {
    if (getenv("DEN_DUMP_HIDDEN") == nullptr) {
        return;
    }
    den_dump_nodes(gf, n_embd);
    static const struct den_dump_tag { const char * prefix; const char * tag; int off; } k_tags[] = {
        { "den_hidden_sumsq", "DEN_DUMP_HIDDEN", 17 }, // full layer output (post-cvec)
        { "den_attn_sumsq",   "DEN_DUMP_ATTN",   15 }, // attention contribution (pre-residual)
        { "den_moe_sumsq",    "DEN_DUMP_MOE",    14 }, // MoE contribution (pre-residual)
        { "den_gdn_sumsq",    "DEN_DUMP_GDN",    14 }, // GDN/SSM contribution (pre-residual)
    };
    int n_matched = 0;
    for (int i = 0; i < gf->n_nodes; ++i) {
        const ggml_tensor * t = gf->nodes[i];
        for (size_t k = 0; k < sizeof(k_tags)/sizeof(k_tags[0]); ++k) {
            const char * p = strstr(t->name, k_tags[k].prefix);
            if (p == NULL || t->ne[0] != 1 || t->ne[1] < 1) {
                continue;
            }
            n_matched++;
            // name is "<prefix>-<il>": skip the "-" so atoi doesn't read a negative
            const int il = atoi(p + k_tags[k].off);
            float sumsq = 0.0f;
            ggml_backend_tensor_get(t, &sumsq, (t->ne[1] - 1)*sizeof(float), sizeof(float));
            const float rms = sqrtf(sumsq / (float) n_embd);
            fprintf(stderr, "%s L%d ntok=%lld rms=%.8f\n", k_tags[k].tag, il, (long long)t->ne[1], rms);
        }
    }
    // Router logits probe: read ffn_moe_logits-L<il> (F32 [n_expert, n_tokens]),
    // compute the top-8 selected expert IDs for the LAST token directly.
    bool rl_seen_l0 = false;
    for (int i = 0; i < gf->n_nodes; ++i) {
        const ggml_tensor * t = gf->nodes[i];
        const char * p = strstr(t->name, "ffn_moe_logits");
        if (p == NULL || t->type != GGML_TYPE_F32 || t->ne[1] < 1) {
            continue;
        }
        const char * p2 = p + 15;              // skip "ffn_moe_logits" + "-"
        if (*p2 < '0' || *p2 > '9') {
            continue;
        }
        const int il = atoi(p2);
        const int n_exp = (int) t->ne[0];
        const int n_tok = (int) t->ne[1];
        std::vector<float> logits((size_t) n_exp);
        ggml_backend_tensor_get(t, logits.data(), (size_t)(n_tok - 1)*n_exp*sizeof(float), (size_t)n_exp*sizeof(float));
        // top-8 by value
        int topk[8]; float topv[8];
        for (int k = 0; k < 8; ++k) { topk[k] = -1; topv[k] = -1e30f; }
        for (int e = 0; e < n_exp; ++e) {
            for (int k = 0; k < 8; ++k) {
                if (logits[e] > topv[k]) {
                    for (int k2 = 7; k2 > k; --k2) { topk[k2] = topk[k2-1]; topv[k2] = topv[k2-1]; }
                    topk[k] = e; topv[k] = logits[e];
                    break;
                }
            }
        }
        ...
        fprintf(stderr, "DEN_LOGIT L%d n_tok=%d exp=[%d,%d,%d,%d,%d,%d,%d,%d] v=[%.4g,%.4g,%.4g,%.4g,%.4g,%.4g,%.4g,%.4g]\n",
                il, n_tok, topk[0], topk[1], topk[2], topk[3], topk[4], topk[5], topk[6], topk[7],
                topv[0], topv[1], topv[2], topv[3], topv[4], topv[5], topv[6], topv[7]);
    }
    // ffn_moe_down probe: [n_embd, n_expert_used, n_tokens] f32. RMS of the
    // LAST token across all expert slots — isolates down-proj vs weighting.
    bool md_seen_l0 = false;
    for (int i = 0; i < gf->n_nodes; ++i) {
        const ggml_tensor * t = gf->nodes[i];
        const char * p = strstr(t->name, "ffn_moe_down");
        if (p == NULL || t->type != GGML_TYPE_F32 || t->ne[2] < 1) {
            continue;
        }
        const char * p2 = p + 13;              // skip "ffn_moe_down" + "-"
        if (*p2 < '0' || *p2 > '9') {
            continue;
        }
        const int il = atoi(p2);
        ...
        const int64_t n_exp = t->ne[1];
        const int64_t n_tot = n_embd * n_exp;
        std::vector<float> all((size_t) n_tot);
        ggml_backend_tensor_get(t, all.data(), (size_t)(n_tok - 1)*t->nb[2], (size_t)n_tot*sizeof(float));
        double ss = 0.0;
        for (int64_t k = 0; k < n_tot; ++k) {
            ss += (double)all[k] * (double)all[k];
        }
        if (il == 0) {
            if (md_seen_l0) {
                continue;
        ...
        fprintf(stderr, "DEN_DOWN L%d n_tok=%lld rms=%.8f\n", il, (long long)n_tok, sqrt(ss / (double)n_tot));
    }
    // ffn_shexp_out probe: shared expert FFN output [n_embd, n_tokens] f32.
    bool sh_seen_l0 = false;
    for (int i = 0; i < gf->n_nodes; ++i) {
        const ggml_tensor * t = gf->nodes[i];
        const char * p = strstr(t->name, "ffn_shexp_out");
        if (p == NULL || t->type != GGML_TYPE_F32 || t->ne[1] < 1) {
            continue;
        }
        const char * p2 = p + 14;              // skip "ffn_shexp_out" + "-"
        if (*p2 < '0' || *p2 > '9') {
            continue;
        }
        const int il = atoi(p2);
        ...
        const int64_t n_embd = t->ne[0];
        std::vector<float> all((size_t) n_embd);
        ggml_backend_tensor_get(t, all.data(), (size_t)(n_tok - 1)*n_embd*sizeof(float), (size_t)n_embd*sizeof(float));
        double ss2 = 0.0;
        for (int64_t k = 0; k < n_embd; ++k) {
            ss2 += (double)all[k] * (double)all[k];
        }
        if (il == 0) {
            if (sh_seen_l0) {
                continue;
        ...
        fprintf(stderr, "DEN_SHEXP L%d n_tok=%lld rms=%.8f\n", il, (long long)n_tok, sqrt(ss2 / (double)n_embd));
    }
    // ffn_moe_weighted probe: routed output [n_embd, n_tokens] f32 (after softmax
    // weights). Last-token RMS — the actual routed contribution to the MoE.
    bool mw_seen_l0 = false;
    for (int i = 0; i < gf->n_nodes; ++i) {
        const ggml_tensor * t = gf->nodes[i];
        const char * p = strstr(t->name, "ffn_moe_weighted");
        if (p == NULL || t->type != GGML_TYPE_F32 || t->ne[1] < 1) {
            continue;
        }
        const char * p2 = p + 17;              // skip "ffn_moe_weighted" + "-"
        if (*p2 < '0' || *p2 > '9') {
            continue;
        }
        const int il = atoi(p2);
        ...
        fprintf(stderr, "DEN_MOE_W L%d n_tok=%lld rms=%.8f\n", il, (long long)n_tok, sqrt(ss3 / (double)n_embd));
    }
    if (n_matched == 0) {
        fprintf(stderr, "DEN_DUMP_NO_MATCH nodes=%d\n", gf->n_nodes);
    }
}
...
static void llama_graph_compute(
        llama_context & lctx,
          ggml_cgraph * gf,
                  int   n_threads) {
    if (getenv("DEN_EVAL_DUMP") != nullptr) {
        // Clean per-node observer: does NOT add graph nodes, so it cannot
        // corrupt the allocator (unlike DEN_DUMP_HIDDEN's ggml_set_output).
        ggml_backend_sched_set_eval_callback(lctx.sched, den_eval_cb, nullptr);
    }
#ifdef GGML_USE_METAL
...
struct llama_context * llama_init_from_model(
    ...
    cparams.expert_cache_mb  = params.expert_cache_mb;
    cparams.expert_prefetch  = params.expert_prefetch;
    ...
    {
        // Always enable expert-offload scheduling: when expert weights are
        ...
        if (cparams.expert_prefetch > 0) {
            LLAMA_LOG_INFO("%s: speculative expert prefetch enabled: lookahead %d\n", __func__, cparams.expert_prefetch);
            ggml_backend_sched_set_expert_prefetch(ctx->sched, cparams.expert_prefetch);
        }
    }
