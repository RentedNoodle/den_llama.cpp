...
void llama_model_qwen35::load_arch_tensors(llama_model_loader & ml) {
    ...
    auto load_block_trunk = [&](int il, int flags) {
        ...
        if (!hparams.is_recr(il)) {
            ...
            layer.wqkv           = create_tensor(tn(LLM_TENSOR_ATTN_QKV,       "weight", il), { n_embd, key_dim * 2 + value_dim }, layer.escha_wqkv.code ? TENSOR_NOT_REQUIRED : flags);
            layer.wqkv_gate      = create_tensor(tn(LLM_TENSOR_ATTN_GATE,      "weight", il), { n_embd, value_dim }, layer.escha_gate.code ? TENSOR_NOT_REQUIRED : flags);
