...
bool create_tensors_helper::create_qwen35_tensors(const LLM_TN & tn) {
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        int flags = 0;
        // Skip loading MTP layers if the feature is disabled; also make them optional
        // since the GGUF may not contain MTP weights at all (upstream converters drop them)
        if (!model.mtp) {
            if (is_mtp_layer) {
                flags |= llama_model_loader::TENSOR_SKIP | llama_model_loader::TENSOR_NOT_REQUIRED;
            }
