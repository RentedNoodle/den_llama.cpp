...
// Den port shims: den's ggml-backend (build 1301a3ac0) predates beellama's
// meta-device query helpers (ggml_backend_dev_is_meta / meta_device_count /
// meta_device_get). They are not declared there and would not link. These
// file-local stand-ins let the KVarN backend-capability queries below compile
// and link unchanged. Conservative behavior: no device is treated as a meta
// device, so the meta-device recursion in the capability queries is inert.
// KVarN is not yet wired into a backend, so this only affects the (unused)
// cache-construction gate; a real port must drop these and use den's ggml meta
// API when it lands.
static inline bool ggml_backend_dev_is_meta(ggml_backend_dev_t dev) {
    GGML_UNUSED(dev);
    return false;
}
static inline size_t ggml_backend_meta_device_count(ggml_backend_dev_t dev) {
    GGML_UNUSED(dev);
    return 0;
}
static inline ggml_backend_dev_t ggml_backend_meta_device_get(ggml_backend_dev_t dev, size_t index) {
    GGML_UNUSED(dev);
    GGML_UNUSED(index);
    return nullptr;
}
