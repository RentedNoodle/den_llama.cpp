...
struct llama_model_loader {
    ...
    struct llama_tensor_weight {
        ...
        llama_tensor_weight(const llama_file * file, uint16_t idx, const char * name, const struct gguf_context * gguf_ctx, ggml_tensor * tensor) : idx(idx), tensor(tensor) {
            ...
            // NVFP4 is stored on-disk at 144 B/256-elem but expanded to 160 B
            // in-memory at load; the file-bounds check must use the on-disk size.
            size_t nbytes = ggml_nbytes(tensor);
            if (tensor->type == GGML_TYPE_NVFP4) {
                nbytes = (size_t) (ggml_nelements(tensor) / ggml_blck_size(tensor->type)) * 144;
            }
            if (offs + nbytes < offs || offs + nbytes > file->size()) {
                ...
                fprintf(stderr, "Error while loading tensor %s: offs = %zu (%zu, %zu), size: %zu, file size: %zu\n", name,
                        offs, data_offset, tensor_offset, nbytes, file->size());
                throw std::runtime_error(format("tensor '%s' data is not within the file bounds, model is corrupted or incomplete", name));
            }
        }

        // .den: on-load data transform / layout flags
        enum xform_t : uint8_t { XFORM_NONE = 0, XFORM_NEG_EXP = 1, XFORM_BF16_TO_F32 = 2 };
        xform_t xform = XFORM_NONE;   // e.g. A_log -> ssm_a = -exp(A_log)
        bool    nvfp4_160b = false;   // .den NVFP4 blocks already in 160B NULLGLASS

        // .den: construct with an explicit absolute file offset (no gguf)
