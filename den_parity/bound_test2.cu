...
static double m_ratio(const float* a, const float* b, int n){ double sa=0,sb=0; for(int i=0;i<n;i++){sa+=fabs(a[i]);sb+=fabs(b[i]);} return sa/sb; }
int main(int argc, char** argv){
    const char* path = argc>1?argv[1]:"attn_qkv_rt.bin";
    const float actscale = argc>2 ? atof(argv[2]) : 1.0f;
    ...
    // N(0,1)-ish via Box-Muller-ish: sum of uniforms
    srand(42); for(int i=0;i<K;i++){ float s=0; for(int j=0;j<6;j++) s+=(float)rand()/RAND_MAX; x[i]=(s-3.0f)*actscale; }
    ...
    printf("act_scale=%.2f  cos=%.6f  mag_ratio(gpu/sw)=%.4f\n", actscale, cos_sim(y_gpu.data(), y_sw.data(), N), m_ratio(y_gpu.data(), y_sw.data(), N));
    return 0;
}
