...
int main(int argc, char** argv){
    ...
    const float scale = argc>2 ? atof(argv[2]) : 1.0f;
    ...
    srand(42); for(int i=0;i<K;i++){ float s=0; for(int j=0;j<12;j++) s+=(float)rand()/RAND_MAX; x[i]=(s-6.0f)*scale; }  // N(0, scale)
    ...
    printf("scale=%.2f cos=%.6f\n", scale, num/sqrt(na*nb2));
    printf("first 8: sw="); for(int i=0;i<8;i++) printf("% .4f ", y_sw[i]); printf("\n");
    printf("first 8: gpu="); for(int i=0;i<8;i++) printf("% .4f ", y_gpu[i]); printf("\n");
    printf("x[0..7]= "); for(int i=0;i<8;i++) printf("% .3f ", x[i]); printf("\n");
    return 0;
}
