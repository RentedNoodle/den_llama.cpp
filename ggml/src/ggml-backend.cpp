...
// ── expert-offload copy engine (den_expert_offload.cu) ───────────────────────
// Host-callable CUDA helpers for the async H2D prefetch stream + VRAM cache pool.
extern "C" {
    int     den_expert_offload_init(int device);
    int     den_expert_h2d_async(int device, void * dst, const void * src, size_t bytes);
    int     den_expert_h2d_sync (int device, void * dst, const void * src, size_t bytes);
    ...
    size_t  den_expert_cache_used(int device);
    void    den_expert_cache_truncate(int device, size_t new_used);
    void    den_expert_offload_shutdown(int device);
}

// cache-pool placement of a resident expert: 64-bit key → (device, offset)
static inline uint64_t ggml_backend_sched_cache_key64(int device, int64_t pool_offset) {
    return ((uint64_t)(uint32_t)device << 32) | (uint64_t)(uint32_t)(pool_offset >= 0 ? pool_offset : 0);
}
...
struct ggml_backend_sched {
    ...
    // ── expert-offload bandwidth levers (Blocker 3) ──────────────────────────
    // VRAM expert LRU cache + speculative async prefetch (see den_expert_offload.cu).
    bool   expert_cache_enabled   = false;   // --expert-cache-mb N (>0 enables)
    size_t expert_cache_bytes     = 0;       // budget (bytes) for resident experts
    bool   expert_prefetch        = false;   // --cpu-moe-prefetch N (>0 enables)
    int    expert_prefetch_depth  = 1;       // lookahead (number of expert splits ahead)

    // resident-expert LRU index, keyed by tensor name (stable across decode calls)
    struct expert_cache_entry {
        std::string key;
        std::vector<uint32_t> resident;      // bitmap, n_expert bits (1 = byte slice cached)
        std::vector<int64_t> slot_off;       // pool offset per expert (-1 = not cached)
        size_t n_expert      = 0;
        size_t expert_size   = 0;            // bytes per expert
        size_t resident_bytes = 0;
        int     cache_dev    = -1;
    };
    std::vector<expert_cache_entry> expert_cache;
    std::vector<std::pair<size_t,int>> expert_lru;  // global LRU: (entry idx, expert id), MRU last

    // speculative routing state (ids of the most recent expert split)
    std::vector<int32_t> spec_ids;
    int spec_n_expert = 0;
    int spec_n_ids    = 0;

    // diagnostics
    bool timing = false;
    uint64_t timing_copy_bytes = 0;
    int64_t  timing_copy_us    = 0;
    int64_t  timing_sync_us    = 0;
    int      timing_expert_splits = 0;
    int      timing_cache_hits    = 0;
    int      timing_cache_misses  = 0;
};
...
// --expert-cache-mb N: cap on the resident hot-expert VRAM cache (0 = disabled).
void ggml_backend_sched_set_expert_cache_mb(ggml_backend_sched_t sched, int mb) {
    if (!sched) return;
    sched->expert_cache_enabled = mb > 0;
    sched->expert_cache_bytes   = mb > 0 ? size_t(mb) * 1024 * 1024 : 0;
}

// --cpu-moe-prefetch N: speculative lookahead for the async expert H2D prefetch (0 = disabled).
void ggml_backend_sched_set_expert_prefetch(ggml_backend_sched_t sched, int lookahead) {
    if (!sched) return;
    sched->expert_prefetch       = lookahead > 0;
    sched->expert_prefetch_depth = lookahead > 0 ? lookahead : 1;
}
        ...
        if (src->buffer != NULL && src->buffer->usage == GGML_BACKEND_BUFFER_USAGE_WEIGHTS) {
            ...
            if (offload_enabled && src_backend_id == sched->n_backends - 1) {
                for (int b = 0; b < src_backend_id; b++) {
                    // ── expert-offload (Blocker 3): when only_active_experts is on,
                    // MoE expert matmuls MUST go to the GPU even for single-token
                    // decode, so copy_inputs can selectively upload the ACTIVE
                    // experts. The vanilla batch heuristic (ggml_backend_cuda_offload_op)
                    // rejects batch=1 (8 active < min_batch), keeping experts on CPU.
                    const bool force_moe_gpu = sched->only_active_experts &&
                        (tensor->op == GGML_OP_MUL_MAT_ID || tensor->op == GGML_OP_MOE_FUSED_UP_GATE) &&
                        src->ne[2] > 1; // multi-expert tensor (ffn_*_exps)
                    if (ggml_backend_supports_op(sched->backends[b], tensor) &&
                            (force_moe_gpu || ggml_backend_offload_op(sched->backends[b], tensor))) {
...
// ── expert-offload LRU cache helpers ─────────────────────────────────────────
// The cache pool lives in den_expert_offload.cu (dedicated VRAM, never aliased).
// Entries are keyed by tensor name so residency survives graph rebuilds across
// decode steps. Only active when sched->expert_cache_enabled.

typedef ggml_backend_sched::expert_cache_entry expert_cache_entry;

static expert_cache_entry * ggml_backend_sched_expert_cache_find(ggml_backend_sched_t sched, const char * key) {
    for (size_t i = 0; i < sched->expert_cache.size(); ++i) {
        if (sched->expert_cache[i].key == key) return &sched->expert_cache[i];
    }
    sched->expert_cache.push_back(expert_cache_entry());
    sched->expert_cache.back().key = key;
    return &sched->expert_cache.back();
}

static size_t ggml_backend_sched_expert_cache_idx(ggml_backend_sched_t sched, const expert_cache_entry * e) {
    return (size_t)(e - sched->expert_cache.data());
}

// ensure the entry has bitmaps sized for n_expert experts of `expert_size` bytes
static void ggml_backend_sched_expert_cache_reserve(ggml_backend_sched_t sched, expert_cache_entry * e,
        size_t n_expert, size_t expert_size) {
    if (e->n_expert == n_expert && e->expert_size == expert_size) return;
    e->n_expert       = n_expert;
    e->expert_size    = expert_size;
    e->resident.assign((n_expert + 31) / 32, 0);
    e->slot_off.assign(n_expert, -1);
    e->resident_bytes = 0;
}

// touch an expert as most-recently-used (global LRU)
static void ggml_backend_sched_expert_cache_touch(ggml_backend_sched_t sched, size_t eidx, int expert_id) {
    auto & lru = sched->expert_lru;
    for (size_t i = 0; i < lru.size(); ++i) {
        if (lru[i].first == eidx && lru[i].second == expert_id) {
            lru.erase(lru.begin() + i);
            break;
        }
    }
    lru.push_back({eidx, expert_id});
}

// evict the single LRU expert (global). Returns true if one was evicted.
static bool ggml_backend_sched_expert_cache_evict_lru(ggml_backend_sched_t sched) {
    if (sched->expert_lru.empty()) return false;
    auto [eidx, expert_id] = sched->expert_lru.front();
    sched->expert_lru.erase(sched->expert_lru.begin());
    if (eidx >= sched->expert_cache.size()) return false;
    auto & e = sched->expert_cache[eidx];
    if ((size_t)expert_id < e.slot_off.size() && e.slot_off[expert_id] >= 0) {
        den_expert_cache_free(e.cache_dev, e.slot_off[expert_id], e.expert_size);
        e.slot_off[expert_id] = -1;
        e.resident[expert_id >> 5] &= ~(1u << (expert_id & 31));
        e.resident_bytes -= e.resident_bytes > e.expert_size ? e.expert_size : e.resident_bytes;
        if (e.resident_bytes == 0 && e.resident.size() > 0) e.resident_bytes = 0;
    }
    return true;
}

// make `expert_id` resident: async H2D from `src_ptr` into the cache pool.
// Returns pool offset (device ptr) if cached, or -1.
static int64_t ggml_backend_sched_expert_cache_put(ggml_backend_sched_t sched, expert_cache_entry * e,
        int expert_id, const uint8_t * src_ptr, int device) {
    if (e->slot_off[expert_id] >= 0) {
        ggml_backend_sched_expert_cache_touch(sched, ggml_backend_sched_expert_cache_idx(sched, e), expert_id);
        return e->slot_off[expert_id];
    }
    int64_t off = den_expert_cache_alloc(device, e->expert_size);
    while (off < 0) {
        if (!ggml_backend_sched_expert_cache_evict_lru(sched)) break;
        off = den_expert_cache_alloc(device, e->expert_size);
    }
    if (off < 0) return -1;
    void * base = den_expert_cache_base(device);
    if (!base) return -1;
    if (den_expert_h2d_async(device, (char*)base + off, src_ptr, e->expert_size) != 0) {
        den_expert_cache_free(device, off, e->expert_size);
        return -1;
    }
    e->slot_off[expert_id] = off;
    e->resident[expert_id >> 5] |= (1u << (expert_id & 31));
    e->resident_bytes += e->expert_size;
    e->cache_dev = device;
    ggml_backend_sched_expert_cache_touch(sched, ggml_backend_sched_expert_cache_idx(sched, e), expert_id);
    return off;
}

static int64_t ggml_backend_sched_expert_cache_get(ggml_backend_sched_t sched, expert_cache_entry * e, int expert_id) {
    if (e->slot_off[expert_id] < 0) return -1;
    ggml_backend_sched_expert_cache_touch(sched, ggml_backend_sched_expert_cache_idx(sched, e), expert_id);
    return e->slot_off[expert_id];
}
...
static void ggml_backend_sched_copy_inputs(ggml_backend_sched_t sched, ggml_backend_sched_split * split, std::array<bool, GGML_SCHED_MAX_BACKENDS> & needs_sync,
    ...
    for (int j = 0; j < split->n_inputs; j++) {
        ...
        if (input->flags & GGML_TENSOR_FLAG_INPUT) {
            ...
            if (getenv("GGML_SCHED_DUMP_ADDR") != nullptr && ggml_backend_buffer_is_host(input->buffer)) {
                fprintf(stderr, "[sched-cond] inp=%s split_has_inputs=%d node_op=%s usage=%d host=%d oae=%d\n",
                        input->name, split->n_inputs > 0 ? 1 : 0,
                        node ? ggml_op_name(node->op) : "none",
                        (int)ggml_backend_buffer_get_usage(input->buffer),
                        ggml_backend_buffer_is_host(input->buffer) ? 1 : 0,
                        sched->only_active_experts ? 1 : 0);
            }
            if (sched->only_active_experts && split->graph.n_nodes > 0 &&
                ...
                if (sched->timing) sched->timing_expert_splits++;

                if (getenv("GGML_SCHED_DUMP_ADDR") != nullptr && sched->timing_expert_splits <= 24) {
                    fprintf(stderr, "[sched-addr] split=%d name=%s ne=(%lld,%lld,%lld) nb2=%lld data=%p host=%p\n",
                            sched->timing_expert_splits, input->name,
                            (long long)input->ne[0], (long long)input->ne[1], (long long)input->ne[2],
                            (long long)(input->ne[2] > 1 ? input->nb[2] : input->nb[1]),
                            (void*)input_cpy->data, (void*)input->data);
                }
                ...
                if (input->ne[2] > 1) {
                    ...
                    // (host → input_cpy) plus an async H2D fill of the cache so
                    // the next decode step hits. The cache pool lives in
                    // den_expert_offload.cu and is never aliased by the graph.
                    const bool use_cache = sched->expert_cache_enabled;
                    ...
                    if (use_cache) {
                        ...
                        cache_base = den_expert_cache_base(cache_dev);
                        // drain the prefetch/fill copy stream so D2D reads below
                        // see fully-written cache bytes (cheap: fills were issued
                        // 1-2 expert splits ago, long finished).
                        den_expert_offload_wait(cache_dev);
                    }

                    auto materialize_expert = [&](int32_t expert_id) {
                        const size_t expert_offset = (size_t)expert_id * expert_size;
                        const size_t padding = 512;
                        const size_t padding_end = expert_id < n_expert - 1 ? std::min<size_t>(expert_size, padding) : 0;
                        const uint8_t * src_host = (const uint8_t *)input->data + expert_offset;

                        if (use_cache && cache_base && ce->slot_off.size() > (size_t)expert_id) {
                            int64_t coff = ggml_backend_sched_expert_cache_get(sched, ce, expert_id);
                            if (coff >= 0) {
                                // cache hit → D2D materialize (exact expert bytes; the
                                // padding region keeps its previous real-data content)
                                if (sched->timing) sched->timing_cache_hits++;
                                den_expert_d2d_async(cache_dev,
                                        (char*)input_cpy->data + expert_offset,
                                        (char*)cache_base + coff,
                                        expert_size);
                                issued_d2d = true;
                            } else {
                                // cache miss → H2D into input_cpy + async cache fill
                                if (sched->timing) { sched->timing_cache_misses++; sched->timing_copy_bytes += expert_size + padding_end; }
                                ggml_backend_tensor_set_async(split_backend,
                                        input_cpy, src_host, expert_offset, expert_size + padding_end);
                                issued_h2d = true;
                                ggml_backend_sched_expert_cache_put(sched, ce, expert_id, src_host, cache_dev);
                            }
                        } else {
                            if (sched->timing) sched->timing_copy_bytes += expert_size + padding_end;
                            ggml_backend_tensor_set_async(split_backend,
                                    input_cpy, src_host, expert_offset, expert_size + padding_end);
                            issued_h2d = true;
                        }
                    ...
                    while (first_id < n_expert) {
                        ...
                        for (int e = first_id; e < last_id; ++e) materialize_expert(e);
                        first_id = next_on_id(last_id);
                    }

                    // make D2D materializations visible before the graph computes.
                    // Sync BOTH the D2D materializations (cache hits) AND the
                    // async H2D cache-miss uploads: the cache-miss path issues
                    // cudaMemcpyAsync on the compute stream but never synced it
                    // (was gated on issued_d2d only), so the graph could read
                    // stale expert weights on a cold start -> garbled decode.
                    // (2026-08-01 fix: expert-offload materialize race)
                    if (issued_d2d || issued_h2d) {
                        den_expert_d2d_wait(cache_dev);
                        if (issued_h2d) {
                            ggml_backend_synchronize(split_backend);
                        }
                    }
...
// true if `split` is an expert-compute split: first node is MUL_MAT_ID /
// MOE_FUSED_UP_GATE consuming a host-pinned expert weight tensor.
static bool ggml_backend_sched_split_is_expert(ggml_backend_sched_t sched, const ggml_backend_sched_split * split) {
    if (split->n_inputs < 1 || split->graph.n_nodes < 1) return false;
    ggml_tensor * node = split->graph.nodes[0];
    if (node->op != GGML_OP_MUL_MAT_ID && node->op != GGML_OP_MOE_FUSED_UP_GATE) return false;
    for (int j = 0; j < split->n_inputs; ++j) {
        ggml_tensor * inp = split->inputs[j];
        if (ggml_backend_buffer_get_usage(inp->buffer) == GGML_BACKEND_BUFFER_USAGE_WEIGHTS &&
...
// Speculative routing: prefetch the NEXT expert split's top experts (using the
// current split's ids as the prediction) into the VRAM cache on the async copy
// stream. Runs right after the current expert split's compute is queued, so the
// H2D overlaps with that layer + the next layer's attention. This never changes
// which experts get materialized later (copy_inputs still copies the exact set)
// — it only pre-warms the cache so the later D2D materialization is a hit.
static void ggml_backend_sched_prefetch_next_experts(ggml_backend_sched_t sched, int i,
        const std::vector<int32_t> & ids) {
    if (ids.empty()) return;
    // find the next expert split
    int j = -1;
    for (int k = i + 1; k < sched->n_splits; ++k) {
        if (ggml_backend_sched_split_is_expert(sched, &sched->splits[k])) { j = k; break; }
    }
    if (j < 0) return;

    const int cache_dev = 0;
    ggml_backend_sched_split * sp = &sched->splits[j];
    for (int l = 0; l < sp->n_inputs; ++l) {
        ggml_tensor * inp = sp->inputs[l];
        if (!(ggml_backend_buffer_get_usage(inp->buffer) == GGML_BACKEND_BUFFER_USAGE_WEIGHTS &&
              ggml_backend_buffer_is_host(inp->buffer))) continue;
        if (inp->ne[2] <= 1) continue;
        const size_t expert_size = inp->nb[2];
        const int n_expert = (int)inp->ne[2];
        expert_cache_entry * ce = ggml_backend_sched_expert_cache_find(sched, inp->name);
        ggml_backend_sched_expert_cache_reserve(sched, ce, (size_t)n_expert, expert_size);
        for (int32_t id : ids) {
            if (id < 0 || id >= n_expert) continue;
            if (ce->slot_off.size() <= (size_t)id || ce->slot_off[id] >= 0) continue;
            ggml_backend_sched_expert_cache_put(sched, ce, id,
                    (const uint8_t*)inp->data + (size_t)id * expert_size, cache_dev);
        }
    }
}
...
static enum ggml_status ggml_backend_sched_compute_splits(ggml_backend_sched_t sched) {
    ...
    const bool timing = sched->timing;
    int64_t tim_total0 = timing ? ggml_time_us() : 0;
    ...
    for (int i = 0; i < sched->n_splits; i++) {
        ...
        int64_t t_copy0 = timing ? ggml_time_us() : 0;
        // copy the input tensors to the split backend
        ggml_backend_sched_copy_inputs(sched, split, sched->needs_sync, ids, unique_ids, last_ids_tensor);
        int64_t t_copy1 = timing ? ggml_time_us() : 0;
        sched->timing_copy_us += t_copy1 - t_copy0;
        ...
        int64_t t_eval0 = timing ? ggml_time_us() : 0;
        auto ec = ggml_backend_sched_eval(sched, split_backend, split);
        if (ec != GGML_STATUS_SUCCESS) {
            return ec;
        }
        if (timing) sched->timing_sync_us += ggml_time_us() - t_eval0;

        // ── speculative expert prefetch ───────────────────────────────────────
        // After this expert split's compute is queued (async), start the H2D
        // prefetch of the NEXT expert split's top experts into the VRAM cache on
        // the copy stream, using THIS split's router ids as the prediction. The
        // transfer overlaps with this layer + the next layer's attention; the
        // later copy_inputs D2D-materializes from the (now warm) cache.
        if (sched->expert_cache_enabled && sched->expert_prefetch &&
                ggml_backend_sched_split_is_expert(sched, split)) {
            ggml_backend_sched_prefetch_next_experts(sched, i, ids);
        }
    ...
    if (timing) {
        int64_t tim_total = ggml_time_us() - tim_total0;
        fprintf(stderr, "[sched-timing] total=%lld us copy=%lld us eval_host=%lld us expert_bytes=%llu expert_splits=%d hits=%d misses=%d n_splits=%d\n",
                (long long)tim_total, (long long)sched->timing_copy_us, (long long)sched->timing_sync_us,
                (unsigned long long)sched->timing_copy_bytes, sched->timing_expert_splits,
                sched->timing_cache_hits, sched->timing_cache_misses, sched->n_splits);
        sched->timing_copy_us = 0; sched->timing_sync_us = 0; sched->timing_copy_bytes = 0;
        sched->timing_expert_splits = 0; sched->timing_cache_hits = 0; sched->timing_cache_misses = 0;
    }
...
ggml_backend_sched_t ggml_backend_sched_new(
    ...
    sched->timing = getenv("GGML_SCHED_TIMING") != NULL;
