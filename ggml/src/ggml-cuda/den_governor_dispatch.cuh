...
#include "nvfp4_omma.cuh"    // OMMA.SF.16864 cubin path (proven tensor cores)
    ...
    float primary_hw_util = 0.0f)
{
    ...
    switch (path) {
        ...
        case ComputePath::SUBVOCAL: {
            // PRIMARY: OMMA.SF.16864 — use the in-library proven kernel
            // (den_mxf4nvf4_gemv_launch, SASS-verified on sm_120a) for the
            // fork's 160B NULLGLASS layout. The standalone cubin
            // (nvfp4_omma_dispatch.cu) now matches the proven Path-A lane
            // mapping (kg = lane%4, single row replicated across all 16
            // M-rows) — verified bit-exact (cos=1.000000) in the isolated
            // cubin GEMV test (tools/den_cubin_gemv_test.cu). Path A stays
            // the runtime primary; the cubin is the drop-in replacement.
            // fused_rmsnorm stays on the in-library path.
            if (M == 1) {
