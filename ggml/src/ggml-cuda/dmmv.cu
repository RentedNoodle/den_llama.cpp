...
void den_set_nvfp4_norm_factors(const std::unordered_map<std::string, std::vector<float>>* m) {
    if (m) {
        ...
        for (auto & kv : *m) {
            if (den_map_dump < 8) {
                fprintf(stderr, "[DEN_DEBUG] norm_map key='%s' n=%zu first=%.9g\n",
                        kv.first.c_str(), kv.second.size(),
                        kv.second.empty() ? 0.0f : kv.second[0]);
                den_map_dump++;
            }
        }
    }
...
const float* den_get_nvfp4_norm_array(const std::string & name, size_t & out_count) {
    ...
    fprintf(stderr, "[DEN_DEBUG] den_get_nvfp4_norm_array: name='%s' n=%zu first=%.9g\n",
            name.c_str(), n_host, n_host ? host_data[0] : 0.0f);
...
static __global__ void dequantize_mul_mat_vec_nvfp4(
    ...
    // Standard E2M1 table (0x8 = -0.0). The DenRaZeR flag was stored in
    // bit 7 of d4[0] -- but d4[0] is a REAL UE4M3 scale byte; any scale
    // >= 0x80 flips the whole block to the wrong table. Removed.
...
// ── Unsloth NVFP4 DMMV — 32 rows per block ──
// Unsloth layout: fp16 d (2B) + 4-bit per-group scales (16B nibble-packed) + qs (128B)
// Scale: d * kscales_nvfp4_unsloth[per_group_scale_nibble]
// Values: kvalues_nvfp4_unsloth (standard E2M1, NOT doubled)
static __global__ void dequantize_mul_mat_vec_nvfp4_unsloth(
    const char * __restrict__ vx, const dfloat * __restrict__ yy, float * __restrict__ dst,
    const int ncols, const int nrows) {
    ...
    const uint8_t * bq = (const uint8_t *) vx;
    const int ncols_qk = ncols / QK_NVFP4;

    const float kvalues[16] = {
    ...
    for (int kb = 0; kb < ncols_qk; kb++) {
        const uint8_t * b = bq + row * ncols_qk * 160 + kb * 160;
        const float d = __half2float(*(const half *)b);  // fp16 at bytes 0-1

        // Per-group 4-bit scales from bytes 2-17
        float scales[16];
        for (int s = 0; s < 16; s++) {
            scales[s] = d * kscales[b[2 + s] & 0xF];
        }

        // Quantized values from bytes 18-145
        const uint8_t * qs = b + 18;
        const int k_off = kb * QK_NVFP4;

        for (int j = 0; j < QK_NVFP4/2; j += 4) {
            uint32_t qw = *(const uint32_t *)(qs + j);
#ifdef GGML_CUDA_F16
            const half * act_h = (const half *)yy + row * ncols;
            half2 a0 = ((const half2 *)act_h)[(k_off + 2*j)/2];
            half2 a1 = ((const half2 *)act_h)[(k_off + 2*j + 2)/2];
            half2 a2 = ((const half2 *)act_h)[(k_off + 2*j + 4)/2];
            half2 a3 = ((const half2 *)act_h)[(k_off + 2*j + 6)/2];
            sumf += kvalues[(qw>> 0)&0xF] * scales[(2*j+0)/16] * __half2float(a0.x);
            sumf += kvalues[(qw>> 4)&0xF] * scales[(2*j+0)/16] * __half2float(a0.y);
            sumf += kvalues[(qw>> 8)&0xF] * scales[(2*j+2)/16] * __half2float(a1.x);
            sumf += kvalues[(qw>>12)&0xF] * scales[(2*j+2)/16] * __half2float(a1.y);
            sumf += kvalues[(qw>>16)&0xF] * scales[(2*j+4)/16] * __half2float(a2.x);
            sumf += kvalues[(qw>>20)&0xF] * scales[(2*j+4)/16] * __half2float(a2.y);
            sumf += kvalues[(qw>>24)&0xF] * scales[(2*j+6)/16] * __half2float(a3.x);
            sumf += kvalues[(qw>>28)&0xF] * scales[(2*j+6)/16] * __half2float(a3.y);
#else
            for (int k = 0; k < 4; k++) {
                const uint8_t bv = (qw >> (k*8)) & 0xFF;
                const int bi = (j+k)*2;
                sumf += kvalues[bv & 0xF] * scales[bi/16]   * yy[row*ncols + k_off + bi];
                sumf += kvalues[bv >> 4]  * scales[(bi+1)/16] * yy[row*ncols + k_off + bi+1];
            }
#endif
        }
    }
...
void ggml_cuda_op_dequantize_mul_mat_vec(
    ...
    switch (src0->type) {
        ...
        case GGML_TYPE_NVFP4: {
            ...
            // Deterministic format detection via tile[148] dispatch byte
            const int format = ggml_nvfp4_detect_format((const uint8_t *)src0_dd_i);

            if (format == 1) { // NULLGLASS 160B — eligible for OMMA dispatch
                ...
                if (den_nvfp4_launch_count < 3) {
                    fprintf(stderr, "[DEN_DEBUG] NVFP4 NULLGLASS: name='%s' tile_norms=%s n_norms=%zu N=%d K=%d kt_per_row=%d\n",
            ...
            } else { // Unsloth 146B — always software DMMV (OMMA dispatch unsupported)
                const int blocks = (N + NVFP4_DMMV_ROWS_PER_BLOCK - 1) / NVFP4_DMMV_ROWS_PER_BLOCK;
                const dim3 block_dims(NVFP4_DMMV_ROWS_PER_BLOCK, 1, 1);
                const dim3 block_nums(blocks, 1, 1);
                dequantize_mul_mat_vec_nvfp4_unsloth<<<block_nums, block_dims, 0, stream>>>(
                    src0_dd_i, src1_dfloat, dst_dd_i, K, N);
            }
        } break;
        default:
