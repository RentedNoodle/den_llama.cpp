...
__global__ void stream_k_decode_nvfp4(
    ...
    for (int kt = 0; kt < kt_per_row; kt++) {
        ...
        #pragma unroll
                ...
                // Self-consistent activation scale (matches the proven M=1 kernel):
                // normalize by the DECODED UE4M3 byte so the OMMA SFB hardware
                // multiply exactly reverses the pre-normalization.
                ...
                uint32_t sfb_packed = 0x01010101u * (uint32_t)sfb_byte;
                uint32_t b0 = 0, b1 = 0;
        #pragma unroll
                for (int i = 0; i < 8; i++) {
                ...
                // Two-pass SFA: rows 0-7 hold row0 weights (own scale bytes),
                // rows 8-15 hold row1 weights (different scale bytes). Scale each
                // row half by ITS OWN bytes in separate OMMA passes.
                uint32_t sfa0 = ((const uint32_t*)tile0)[mm];
                uint32_t sfa1 = ((const uint32_t*)tile1)[mm];

                float d0, d1, d2, d3;
                // Pass 1: row0 scales -> rows 0-7 (row0)
                OMMA_MXF4NVF4_4X(d0, d1, d2, d3,
                    a0, a1, a2, a3, b0, b1,
                    acc0, acc1, 0.0f, 0.0f,
                    sfa0, sfb_packed);
                acc0 = d0; acc1 = d1;
                // Pass 2: row1 scales -> rows 8-15 (row1)
                OMMA_MXF4NVF4_4X(d0, d1, d2, d3,
                    a0, a1, a2, a3, b0, b1,
                    0.0f, 0.0f, acc2, acc3,
                    sfa1, sfb_packed);
...
__global__ void mid_batch_gemm_nvfp4(
    ...
    // Per-row RMSNorm of the activation (identical for every output tile).
    // Computed once per row; each lane covers a disjoint K/32 slice.
    float rms_scale_f = 1.0f;
    if (fused_rmsnorm) {
        float rss = 0.0f;
        for (int k = lane_in_warp; k < K; k += 32) {
            float v = x_row[k];
            rss += v * v;
        }
        #pragma unroll
        for (int mask = 1; mask < 32; mask <<= 1) {
            rss += __shfl_xor_sync(0xffffffff, rss, mask);
        }
        rms_scale_f = rsqrtf(rss / K + rms_eps);
    }

    // One CTA per M-row. Each warp walks 16-row output TILES in N (2 warps
    // stride 2 tiles) — non-overlapping coverage.  Every lane stays converged
    // for the whole tile body: out-of-range rows are clamped and writes masked,
    // so the block_max __shfl_xor_sync(0xffffffff,...) below is always legal.
    for (int ntile = warp_id; ntile * 16 < N; ntile += (blockDim.x / 32)) {
        const int base  = ntile * 16;
        const bool valid0 = (base + r) < N;
        const bool valid1 = (base + r + 8) < N;
        const int row0 = valid0 ? (base + r) : base;
        const int row1 = valid1 ? (base + r + 8) : base;

        float total0 = 0.0f, total1 = 0.0f;
        float total2 = 0.0f, total3 = 0.0f;

        const uint8_t* row0_tiles = w + (size_t)row0 * row_stride;
        const uint8_t* row1_tiles = w + (size_t)row1 * row_stride;

        for (int kt = 0; kt < kt_per_row; kt++) {
            float acc0 = 0.0f, acc1 = 0.0f, acc2 = 0.0f, acc3 = 0.0f;

            const uint8_t* tile0 = row0_tiles + kt * BYTES_PER_TILE;
            const uint8_t* tile1 = row1_tiles + kt * BYTES_PER_TILE;

            // Null-tile pruning: mark for skip, but NEVER `continue` — that
            // would diverge the warp and make the block_max shuffle below UB.
            const bool null_tile =
                den_tile_is_null(bvh, row0 * kt_per_row + kt, (const uint32_t*)tile0) &&
                den_tile_is_null(bvh, row1 * kt_per_row + kt, (const uint32_t*)tile1);
            if (null_tile && rt_skipped && threadIdx.x == 0) atomicAdd(rt_skipped, 1ULL);
            ...
            for (int mm = 0; mm < 4; mm++) {
                ...
                // Activation load + dynamic E2M1 quantization.  Runs for EVERY
                // lane (no divergence) so the shuffle reduction is converged.
                int kb_lo = kt * 256 + mm * 64 + kg * 8;
                int kb_hi = kt * 256 + mm * 64 + 32 + kg * 8;
                float x_local[16];
                float local_max = 0.0f;
...
#pragma unroll
                ...
                // Self-consistent activation scale (matches the proven M=1
                // kernel): normalize by the DECODED UE4M3 byte so the OMMA
                // SFB hardware multiply exactly reverses the normalization.
                float sfb_f = fmaxf(0.0625f, fminf(1.875f, block_max * 0.25f));
                uint8_t sfb_code = quant_f32_ue4m3(sfb_f);
                uint8_t sfb_byte = ue4m3_code_to_byte[sfb_code];
                float sfb_val = ue4m3_byte_to_f32(sfb_byte);
...
#pragma unroll
                ...
                if (!null_tile) {
                    // Two-pass SFA: the OMMA SFA register scales ALL 16 M-rows,
                    // but rows 0-7 hold row0 weights (own scale bytes) and rows
                    // 8-15 hold row1 weights (different scale bytes). Scale each
                    // row half by ITS OWN bytes in separate OMMA passes.
                    uint32_t sfa0 = ((const uint32_t*)tile0)[mm];
                    uint32_t sfa1 = ((const uint32_t*)tile1)[mm];
        ...
        if (kg == 0) {
            float* y_row = y + (size_t)row * N;
            if (valid0) y_row[base + r] = total0 * rms_scale_f;
            if (valid1) y_row[base + r + 8] = total2 * rms_scale_f;
        }
    }
}
