...
struct alignas(16) TileData {
    ...
    uint32_t sfa[4];  // scale factor A (row0)  (((const uint32_t*)tile0)[mm])
    uint32_t sfa1[4]; // scale factor A for row1-half (row1's tile1 scale bytes)
    float norm0;     // tile_norm (global scale) for row0, header bytes 152:155
    float norm1;     // tile_norm (global scale) for row1, header bytes 152:155
#ifdef DENSCALE_V
    uint8_t  coarse[8];  // DenScale-V: 8 UE8M0 coarse scales (tile bytes 128-135)
    uint8_t  coarse1[8]; // DenScale-V: row1-half coarse scales (tile1 bytes 128-135)
#endif
    ...
    int tile_bytes = 160)   // NULLGLASS: 160B tiles with 16B cognitive header
{
    ...
    for (int mm = 0; mm < 4; mm++) {
        // NULLGLASS 160B in-memory is INTERLEAVED (matches the loader + CPU oracle
        // dequantize_row_nvfp4): sub-block mm's 32 nibble bytes at 36*mm+4, its
        // 4 UE4M3 scale bytes at 36*mm. The 152B DENSCALE_V path stays legacy.
        const uint32_t * q0 = (const uint32_t *)(tile0 + (tile_bytes == 160 ? 36*mm + 4 : nib_offset + mm*32));
        const uint32_t * q1 = (const uint32_t *)(tile1 + (tile_bytes == 160 ? 36*mm + 4 : nib_offset + mm*32));
        td.a0[mm] = __ldg(&q0[kg]);
        td.a2[mm] = __ldg(&q0[4 + kg]);
        td.a1[mm] = __ldg(&q1[kg]);
        td.a3[mm] = __ldg(&q1[4 + kg]);
        td.sfa[mm]  = __ldg(&((const uint32_t *)(tile0 + (tile_bytes == 160 ? 36*mm : sfa_offset)))[tile_bytes == 160 ? 0 : mm]);
        td.sfa1[mm] = __ldg(&((const uint32_t *)(tile1 + (tile_bytes == 160 ? 36*mm : sfa_offset)))[tile_bytes == 160 ? 0 : mm]);
    }
#ifdef DENSCALE_V
    ...
    // OMMA_NATIVE_FUSED_SCALE: unified per-tile norm at header bytes 152:155
    // (float32). Written per-expert/per-tensor by the loader at expansion.
    // Loaded here so ACCUMULATE multiplies the tile's own scale — no separate
    // norm array, no per-row*kt index math (kills the MoE OOB bug).
    memcpy(&td.norm0, tile0 + 152, 4);
    memcpy(&td.norm1, tile1 + 152, 4);
}
    ...
    for (int kt = 0; kt < kt_per_row; kt++) {
        ...
        for (int mm = 0; mm < 4; mm++) {
            ...
            if constexpr (INPUT_IS_E2M1) {
                ...
                float sfb_f = fmaxf(0.0625f, fminf(1.875f, block_max * 0.25f));
...
#ifdef DEN_USE_CONSTANT_SCALE_TABLE
                {
                    ...
                    // NOTE: sfb is the ACTIVATION scale ONLY. The OMMA pipeline is
                    // A_e2m1 x B_e2m1 x SFA x SFB: the hardware applies the weight
                    // scale SFA via bufA.sfa, so the B-fragment scale must NOT fuse
                    // it. den_scale_product(sfa,sfb) was wrong twice: (1) it double
                    // applied the weight scale, (2) real weight-scale bytes (e.g.
                    // 0x64=48.0) are outside the 16 canonical UE4M3 codes and decoded
                    // to 0, collapsing sfb_f to 0.0625 and clipping activations.
                    uint8_t sfb_code_mod = quant_f32_ue4m3(fmaxf(0.0625f,
                        fminf(1.875f, sfb_f * g_personality_scale)));
                    // byte -> float from constant cache in ~2 cycles (no product)
                    sfb_f = __ldca(&g_ue4m3_full_decode[ue4m3_code_to_byte[sfb_code_mod]]);
                ...
                // Self-consistent activation scale: quantize sfb to its UE4M3 byte,
                // then normalize the activations by the DECODED byte value — the same
                // scale the OMMA hardware multiplies (SFB). Normalizing by the ideal
                // block_max/N while multiplying by the quantized byte biased the
                // reconstruction ~16% high (sum-ratio 1.16 vs software); using the
                // decoded byte for both leaves only the E2M1 rounding error (~1.0).
                uint8_t sfb_code = quant_f32_ue4m3(sfb_f);
                uint8_t sfb_byte = ue4m3_code_to_byte[sfb_code];
                float sfb_val = ue4m3_byte_to_f32(sfb_byte);
                sfb_val = fmaxf(0.0625f, fminf(1.875f, sfb_val));
                float sfb_inv = 1.0f / sfb_val;
                sfb_packed = 0x01010101u * (uint32_t)sfb_byte;
                b0 = 0; b1 = 0;
                #pragma unroll
                for (int i = 0; i < 8; i++) {
            ...
            // OMMA with pre-loaded A-fragments and scale from bufA — TWO passes.
            // The OMMA SFA register is a per-16-K scale vector shared by ALL 16
            // M-rows, but the A-fragment rows 0-7 hold row0's weights and rows
            // 8-15 hold row1's, each with its OWN per-16-K scale bytes. A single
            // pass scales both halves by row0's bytes, corrupting rows 8-15
            // (sum-ratio 1.20, ~5% sign flips concentrated in the row1 half).
            //   Pass 1 (SFA = row0's scales): D rows 0-7 → acc0/acc1 = row0.
            //   Pass 2 (SFA = row1's scales): D rows 8-15 → acc2/acc3 = row1.
            if constexpr (THERMAL_MEMORY) {
            ...
            // Applied per row-half with ITS OWN scales: row0 from bufA.sfa/coarse,
            // row1 from bufA.sfa1/coarse1 (matching the two-pass SFA split).
            if (tile_bytes == 152) {
                uint8_t fine_code0 = (uint8_t)((bufA.sfa[mm] >> (kg * 8)) & 0xFF);
                uint8_t coarse_code0 = bufA.coarse[mm * 2 + (kg >> 1)];
                float fine_scale0 = (float)ue4m3_code_to_byte[fine_code0 & 0xF]
                                    / 255.0f * 6.0f;
                float coarse_scale0 = (float)coarse_code0 / 32.0f;
                float corr0 = fine_scale0 / (coarse_scale0 + 1e-10f);
                acc0 *= corr0; acc1 *= corr0;

                uint8_t fine_code1 = (uint8_t)((bufA.sfa1[mm] >> (kg * 8)) & 0xFF);
                uint8_t coarse_code1 = bufA.coarse1[mm * 2 + (kg >> 1)];
                float fine_scale1 = (float)ue4m3_code_to_byte[fine_code1 & 0xF]
                                    / 255.0f * 6.0f;
                float coarse_scale1 = (float)coarse_code1 / 32.0f;
                float corr1 = fine_scale1 / (coarse_scale1 + 1e-10f);
                acc2 *= corr1; acc3 *= corr1;
            }
#endif
        }
        ...
        if (!bufA.policy.null_skip) {
            if (kg == 0) {
                // OMMA_NATIVE_FUSED_SCALE: prefer the per-tile norm folded into
                // the header (bytes 152:155). Fall back to the legacy tile_norms
                // array only when the header holds 1.0 (no .scale companion).
                float n0 = bufA.norm0, n1 = bufA.norm1;
                if ((n0 == 1.0f || n1 == 1.0f) && tile_norms) {
                    if (n_norms == 1) {
...
// ─────────────────────────────────────────────────────────────────────────
// den_nvfp4_soft_gemv — SOFTWARE NVFP4 dequant GEMV (matches the coherent CPU
// dequant path).  Used for the attention projections and the lm_head, where the
// OMMA kernels' 4-bit E2M1 activation quantization is too coarse: the GDN
// recurrence amplifies it (hidden states diverge) and the lm_head argmax flips.
//
// Dequant formula (identical to dequantize_row_nvfp4 in iqk_quantize.cpp):
//   weight = ue4m3(scale_byte) * norm[152:155] * e2m1_std(nibble)
// where e2m1_std is the STANDARD E2M1 table — the GGUF nibbles are "doubled"
// E2M1 and the CPU path's *0.5 cancels that, so standard*scale == 0.5*doubled*scale.
// ─────────────────────────────────────────────────────────────────────────
static __global__ void den_nvfp4_soft_gemv_kernel(
    const uint8_t* __restrict__ w,   // NULLGLASS 160B tiles
...
static void den_nvfp4_soft_gemv_launch(
    const void* weights, const float* act, float* dst,
    int N, int K, cudaStream_t stream)
{
    const int blocks = (N + 31) / 32;
    den_nvfp4_soft_gemv_kernel<<<blocks, 32, 0, stream>>>(
