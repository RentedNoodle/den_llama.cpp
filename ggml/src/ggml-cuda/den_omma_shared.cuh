...
static __device__ __forceinline__ uint8_t quant_f32_e2m1(float fv) {
    ...
    else if (av >= 0.25f) n = 1;   // midpoint of E2M1 codes 0 (0.0) and 1 (0.5)
    if (sign) n |= 8;
    return n;
}
...
// UE4M3 byte → float32 decode (E4M3 unsigned, matches ggml_cuda_ue4m3_to_fp32).
// Used to normalize activations by the same scale value the OMMA SFB hardware
// multiplies (self-consistent sfb → unbiased sum-ratio ~1.0).
__device__ __forceinline__ float ue4m3_byte_to_f32(uint8_t byte) {
    if (byte >= 0x7F) return 0.0f;
    int exp = (byte >> 3) & 0x0F;
    int mant = byte & 0x07;
