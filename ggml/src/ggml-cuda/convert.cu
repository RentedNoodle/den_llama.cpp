...
static __device__ const float kvalues_nvfp4_unsloth[16] = {
    0.0f, 0.5f, 1.0f, 1.5f, 2.0f, 3.0f, 4.0f, 6.0f,
   -0.0f,-0.5f,-1.0f,-1.5f,-2.0f,-3.0f,-4.0f,-6.0f
};
static __device__ const float kscales_nvfp4_unsloth[16] = {
...
template<typename dst_t>
static __global__ void dequantize_block_nvfp4_unsloth(const void * vx, dst_t * yy) {
    const int64_t i = blockIdx.x, tid = threadIdx.x;
    const uint8_t * blk = (const uint8_t *)vx + i * sizeof(block_nvfp4);
    const float d = __half2float(*(const half *)blk);
    const float s = kscales_nvfp4_unsloth[blk[2 + tid] & 0xF];
    dst_t * y = yy + i * QK_NVFP4 + tid * 16;
    const uint8_t * qs = blk + 18;
    for (int j = 0; j < 8; j++) {
        const uint8_t q = qs[tid * 8 + j];
        y[j] = d * s * kvalues_nvfp4_unsloth[q & 0x0F];
        y[j+8] = d * s * kvalues_nvfp4_unsloth[q >> 4];
    }
}
...
// ── Unsloth NVFP4 format LUTs (standard E2M1 + UE4M3, NOT doubled) ──
...
static __global__ void dequantize_block_nvfp4(const void * __restrict__ vx, dst_t * __restrict__ yy) {
    ...
    float norm = 1.0f;
    memcpy(&norm, x->padding + 8, 4);
    // GGUF convention: value = ue4m3(scale)*0.5 * KVAL_doubled[nib] = standard
    // E2M1. Must match dmmv.cu + OMMA (verified cos 0.99996). Missing 0.5 = 2x.
...
static void dequantize_row_nvfp4_cuda(const void * vx, dst_t * y, const int64_t nrows, const int64_t n_per_row, cudaStream_t stream) {
    ...
    const int format = ggml_nvfp4_detect_format((const uint8_t *)vx);

    if (format == 0) { // Unsloth 146B (zero-padded to 160B)
        dequantize_block_nvfp4_unsloth<<<nb, 16, 0, stream>>>(vx, y);
    } else { // NULLGLASS 160B
