...
//
// Tile layout (160B NULLGLASS, matching den_llama.cpp block_nvfp4):
//   [0:15]     — UE4M3 block scales (16 bytes, 4x uint32 packed per sub-tile)
//   [16:143]   — E2M1 weight nibbles (256 elements × 4 bits = 128 bytes)
//   [144:147]  — padding / null_skip policy byte (bit 7 of byte 144)
//   [148]      — format/dispatch byte (0x10=GEMV, 0x30=SW_DQ, 0x50=MoE_SPARSE)
//   [149]      — flags (8=WH4 v1 needs /16 correction, 9=WH4 v2 pre-folded)
//   [150:151]  — reserved
//   [152:155]  — tile_norm (float32, per-tensor/per-expert, folded at load)
//   [156:159]  — reserved
// ═══════════════════════════════════════════════════════════════════════════════
    ...
    int N, int K, int tpr)               // rows, cols, tiles per row
{
    ...
    int kg = lane & 3;   // K-group 0-3 within each 64-K sub-block (Path A proven: kg = lane%4)
    ...
    for (int t = 0; t < tpr; t++) {
        ...
        for (int sub = 0; sub < 4; sub++) {
            int k_off = t * 256 + sub * 64;   // activation K-offset (tile t covers K[t*256 : (t+1)*256))

            // ── Build A fragment (weight E2M1 nibbles → uint32 registers) ──
            // Path A proven layout: the single weight row is replicated across all
            // 16 M-rows of the OMMA. Lane kg = lane%4 loads K[kg*8 : kg*8+8) into
            // a0/a1 (rows 0-7 / 8-15) and K[32+kg*8 : 40+kg*8) into a2/a3. The four
            // kg-lanes tile K[0:64); the tensor core reduces them into d0..d3.
            const uint32_t* q = (const uint32_t*)(tile + 16 + sub * 32);
            uint32_t a0 = q[kg];        // K[kg*8 : kg*8+8)       — rows 0-7
            uint32_t a2 = q[4 + kg];    // K[32+kg*8 : 40+kg*8)   — rows 0-7
            uint32_t a1 = a0;           // rows 8-15 = same single row
            uint32_t a3 = a2;
    ...
    int N, int K, int tpr)
{
    ...
    for (int t = 0; t < tpr; t++) {
        ...
        for (int sub = 0; sub < 4; sub++) {
            ...
            // A fragment: single weight row replicated across all 16 M-rows.
            // Lane kg = lane%4 tiles K[0:64); a0/a1 = K[kg*8:kg*8+8), a2/a3 = K[32+kg*8:40+kg*8).
            const uint32_t* q = (const uint32_t*)(tile + 16 + sub * 32);
            uint32_t a0 = q[kg];
            uint32_t a2 = q[4 + kg];
            uint32_t a1 = a0;
