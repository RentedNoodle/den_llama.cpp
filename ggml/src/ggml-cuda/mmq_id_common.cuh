...
#pragma unroll
        ...
        // id-MMQ path reads only d4[] ue4m3 scales; without this fold the
        // expert mul_mat_id output is 1/norm too large.  On sm_120a the TURING
        // branch of vec_dot_q8_0_16_q8_1_mma does NOT halve the q8_1 activation
        // scale (only the AMD branch does), so the 0.5 must cancel the doubled
        // kvalues_mxfp4 E2M1 table here: x_df = ue4m3(scale)*norm*0.5 matches
        // dequantize_row_nvfp4  ue4m3(scale)*norm*0.5*kvalues_mxfp4[nibble].
...
#else
        x_df[i*(2*MMQ_TILE_NE_K*2/QI8_0) + i/(QI8_0/4) + 2*kqsx + 0] = s0n;
        x_df[i*(2*MMQ_TILE_NE_K*2/QI8_0) + i/(QI8_0/4) + 2*kqsx + 1] = s1n;
#endif
    }
}
