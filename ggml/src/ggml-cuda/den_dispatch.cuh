    ...
    bool fused_rmsnorm = false, float rms_eps = 1e-6f)
{
    ...
    static int den_dispatch_count = 0;
    if (den_dispatch_count < 5) {
        fprintf(stderr, "[DEN_DEBUG] dispatch->OMMA GEMV launch: N=%d K=%d tile_norms=%s n_norms=%d fused_rmsnorm=%d\n",
                N, K, tile_norms ? "VALID" : "nullptr", n_norms, (int)fused_rmsnorm);
        den_dispatch_count++;
    }
    den_mxf4nvf4_gemv_launch(weights, act, dst, N, K, stream, tile_norms, n_norms, fused_rmsnorm, rms_eps);
}
