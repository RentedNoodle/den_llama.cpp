...
static void ggml_cuda_flash_attn_ext_nvfp4_kv(ggml_backend_cuda_context & ctx, ggml_tensor * dst, int il) {
    // The Den NVFP4-KV attention kernel is decode-only (writes a single query token,
    // one block per head). For multi-token prefill it would leave tokens 1..n-1 of the
    // output buffer unwritten -> uninitialized memory read by the fused gate sigmoid.
    // Fall back to the standard vec kernel for any batch larger than one token.
    if (dst->src[0]->ne[1] != 1) {
