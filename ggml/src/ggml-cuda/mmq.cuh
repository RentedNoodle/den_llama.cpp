...
// CUDA 13.3
#ifndef DEN_L2_AFFINITY_TABLE_SIZE
#define DEN_L2_AFFINITY_TABLE_SIZE 1
#endif
...
#pragma unroll
...
#else
            x_qs[i*(2*WARP_SIZE + 1)     + 4*grp + (l&1) + 0] = v.x;
            x_qs[i*(2*WARP_SIZE + 1)     + 4*grp + 2 + (l&1)] = v.y;
#endif
        }
        ...
        // per-expert norm folded into NULLGLASS header bytes 152:155.  The GPU
        // MMQ paths read only d4[] ue4m3 scales; without this fold the expert
        // mul_mat_id output is 1/norm too large (rms 5.5e12 vs 0.011 CPU).
        // On sm_120a the TURING branch of vec_dot_q8_0_16_q8_1_mma does NOT
        // halve the q8_1 activation scale, so the 0.5 must cancel the doubled
        // kvalues_mxfp4 E2M1 table here.
        float nvfp4_norm = 1.0f;
        memcpy(&nvfp4_norm, bxi->padding + 8, 4);
        const float nvfp4_scl = (nvfp4_norm == 0.0f) ? 0.5f : 0.5f * nvfp4_norm;
        const float s0n = s0 * nvfp4_scl;
        const float s1n = s1 * nvfp4_scl;

#ifdef INT8_MMA_AVAILABLE
        x_df[i*MMQ_MMA_TILE_X_K_Q3_K + 2*kqsx + 0] = s0n;
        x_df[i*MMQ_MMA_TILE_X_K_Q3_K + 2*kqsx + 1] = s1n;
#else
        x_df[i*(2*WARP_SIZE*2/QI8_0) + i/(QI8_0/4) + 2*kqsx + 0] = s0n;
        x_df[i*(2*WARP_SIZE*2/QI8_0) + i/(QI8_0/4) + 2*kqsx + 1] = s1n;
#endif
    }
}
