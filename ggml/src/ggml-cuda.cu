...
#include "ggml-cuda/nvfp4_omma.cuh"
...
extern "C" void den_scale_table_init(void) {
    ...
    // TRUE UE4M3 (E4M3 unsigned) decode — MUST match ggml_cuda_ue4m3_to_fp32
    // in common.cuh (the cos-verified software path). The previous linear
    // table was WRONG for bytes 0x39-0x3F: e.g. 0x3E = (1+6/8)*2^7 = 1.75,
    // not 1.375. Every high-magnitude scale was corrupted.
...
static int ggml_cuda_mul_mat_q(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst,
    ...
    if (is_gemv) {
        // ── NVFP4 OMMA fast path: use HW tensor cores for single-token decode ──
        ...
        if (src0->type == GGML_TYPE_NVFP4) {
            ...
            bool is_bias_fusion = (!is_qkv_fusion && fusion && cgraph &&
            ...
            if (!is_qkv_fusion && !is_bias_fusion) {
                // Simple single-token decode: convert f32 → f16, launch OMMA cubin
                int64_t ne10 = src1->ne[0];
                int64_t ne11 = src1->ne[1];
                int64_t ne12 = src1->ne[2];
                int64_t n_activations = ne10 * ne11 * ne12;

                ggml_cuda_pool_alloc<half> src1_f16(ctx.pool(), n_activations);
                nvfp4_f32_to_f16((const float *)src1->data, src1_f16.get(),
                                  (int)n_activations, stream);
                CUDA_CHECK(cudaGetLastError());
                ...
                fprintf(stderr, "nvfp4_omma: routing single-token decode through OMMA.SF.16864 "
                        "(N=%d rows, K=%d cols)\n", args.nrows_x, args.ncols_x);

                mul_mat_vec_nvfp4_cuda(args, stream);
                CUDA_CHECK(cudaGetLastError());

                ++node_n;
                return node_n;
            }
            // Fusion cases: fall through to software dequant path below
        }
...
static int ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst,
    ...
    if (any_gpus_with_slow_fp16 && src0->type == GGML_TYPE_F16 && ggml_is_permuted(src0) && ggml_is_permuted(src1) && src1->ne[1] == 1) {
        ...
        // recurrence (hidden states diverge ~4x), and for the lm_head it flips the
        ...
        static int den_norm_lookup_count = 0;
        if (den_norm_lookup_count < 8) {
            fprintf(stderr, "[DEN_DEBUG] norm_lookup name='%s' found=%d n_norms=%zu\n",
                    src0->name, tile_norms ? 1 : 0, n_norms);
            den_norm_lookup_count++;
        }
        cudaStream_t stream = ctx.stream();
    ...
    switch (dst->op) {
        ...
        case GGML_OP_RMS_NORM:
            ...
            if (fusion && next && next->op == GGML_OP_MUL_MAT &&
                ...
                // Do NOT fuse for attention/lm_head — those NVFP4 GEMVs use the
                // software dequant path which needs the normed input in src1.
                if (strstr(next->src[0]->name, "attn") == nullptr &&
                    strstr(next->src[0]->name, "output.weight") == nullptr) {
...
GGML_CALL static enum ggml_status ggml_backend_cuda_graph_compute(ggml_backend_t backend, ggml_cgraph * cgraph) {
    ...
    const char * den_use_graphs_env = getenv("GGML_CUDA_USE_GRAPHS");
    static const bool disable_cuda_graphs_due_to_env = (getenv("GGML_CUDA_DISABLE_GRAPHS") != nullptr)
        || (den_use_graphs_env != nullptr && den_use_graphs_env[0] == '0');
...
GGML_CALL static bool ggml_backend_cuda_supports_op(ggml_backend_t backend, const ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_FLASH_ATTN_EXT:
            {
                // CUDA flash attention (fattn.cu) supports f16/bf16/f32 KV caches. Claiming this
                // op keeps attention on the GPU instead of falling to the CPU IQK path, which reads
                // host-side copies of the GPU KV cache (garbage for large GPU-resident caches).
