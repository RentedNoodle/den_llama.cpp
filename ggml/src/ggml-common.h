...
// ── NVFP4 format variant detection ──
// Two NVFP4 block formats exist, both stored in 160-byte blocks:
//   NULLGLASS (ours):   bytes 0-15=d4[4] UE4M3 scales, 16-143=qs[128] E2M1, 144-159=padding
//                       padding[4] (=byte 148) = dispatch byte: 0x10=GEMV, 0x30=SW_DQ, 0x50=MoE_SPARSE
//   Unsloth:            bytes 0-1=fp16 d, 2-17=4-bit per-group scales (16 nibble-packed bytes),
//                       18-145=qs[128] standard E2M1, 146-159=zero padding
// DETECTION (deterministic — no fp16 heuristic):
//   Read byte 148 of the first block. NULLGLASS dispatch bytes (0x10, 0x30, 0x50)
//   never appear in Unsloth's zero padding (always 0x00).

#define NVFP4_DISPATCH_GEMV      0x10
#define NVFP4_DISPATCH_SW_DQ     0x30
#define NVFP4_DISPATCH_MOE_SPARSE 0x50

#define NVFP4_BLOCK_BYTES   160
#define NVFP4_UNSLOTH_BYTES 146  // meaningful data within 160B block

static inline int ggml_nvfp4_detect_format(const uint8_t * data) {
    // NULLGLASS dispatch byte at tile[148] is authoritative when present.
    uint8_t d = data[148];
    if (d == NVFP4_DISPATCH_GEMV || d == NVFP4_DISPATCH_SW_DQ || d == NVFP4_DISPATCH_MOE_SPARSE) {
        return 1; // NULLGLASS 160B dispatch header
    }
    // Otherwise default to the d4-first scale layout used by standard
    // modelopt->GGUF NVFP4 conversions (16 scale bytes at 0:16, quants at
    // 16:144). Only route to the Unsloth layout (fp16 norm at 0:2, scales at
    // 2:18, quants at 18:146) when the block provably matches it: the
    // trailing 146:160 region is all-zero padding AND bytes 0:2 form a
    // finite fp16 block norm (not two arbitrary d4 scale bytes).
    const uint16_t nb = (uint16_t)(data[0] | (data[1] << 8));
    const uint32_t fexp = (nb >> 10) & 0x1F;
    if (fexp != 0x1F) { // not NaN/Inf
        int zero_pad = 1;
        for (int i = 146; i < 160; ++i) {
            if (data[i] != 0) { zero_pad = 0; break; }
        }
        if (zero_pad) {
            return 0; // Unsloth 146B (zero-padded to 160B)
        }
    }
    return 1; // NULLGLASS / standard d4-first layout
}
...
// ── TurboQuant KV cache types (from BeeLlama) ──
// All block sizes are 128 elements.
#define QK_TURBO2_0 128
#define QK_TURBO3_0 128
#define QK_TURBO4_0 128
#define QK_TURBO2_TCQ 128
#define QK_TURBO3_TCQ 128
#define QK_TURBO4_TCQ 128

typedef struct {
    ggml_half d;           // fp16 block norm
    uint8_t   qs[64];      // 2-bit indices (128 * 2 / 8 * 2  →  32B after QJL)
    // NOTE: simplified — full BeeLlama struct has QJL bitstream
} block_turbo2_0;
static_assert(sizeof(block_turbo2_0) == 66, "wrong turbo2_0 block size/padding");

typedef struct {
    ggml_half d;           // fp16 block norm
    uint8_t   qs[64];      // 3-bit: 2-bit PolarQuant (32B) + 1-bit QJL (16B) = 48B, padded
} block_turbo3_0;
static_assert(sizeof(block_turbo3_0) == 66, "wrong turbo3_0 block size/padding");

typedef struct {
    ggml_half d;           // fp16 block norm
    uint8_t   qs[64];      // 4-bit indices (128 * 4 / 8 = 64B)
} block_turbo4_0;
static_assert(sizeof(block_turbo4_0) == 66, "wrong turbo4_0 block size/padding");

typedef struct {
    ggml_half d;           // fp16 block norm
    uint8_t   qs[49];      // TCQ bitstream (k=2, L=8, 256 states)
    uint8_t   pad;
} block_turbo2_tcq;
static_assert(sizeof(block_turbo2_tcq) == 52, "wrong turbo2_tcq block size/padding");

typedef struct {
    ggml_half d;           // fp16 block norm
    uint8_t   qs[49];      // TCQ bitstream (k=3, L=9, 512 states)
    uint8_t   pad;
} block_turbo3_tcq;
static_assert(sizeof(block_turbo3_tcq) == 52, "wrong turbo3_tcq block size/padding");

typedef struct {
    ggml_half d;           // fp16 block norm
    uint8_t   qs[65];      // TCQ bitstream (k=4, L=10, 1024 states)
    uint8_t   pad;
} block_turbo4_tcq;
static_assert(sizeof(block_turbo4_tcq) == 68, "wrong turbo4_tcq block size/padding");
