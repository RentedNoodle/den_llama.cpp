...
extern "C" IQK_API bool iqk_moe_fused_up_gate(long Nx, long Ny, long ne00, int ne11, int unary_op,
    ...
    if (etypeA == GGML_TYPE_NVFP4) {
        // Re-pack as Q8_0_R8 rows so the IQK kernel pipeline can process it.
        // ne00 can be much larger than QK_NVFP4 (e.g. 2048 for the 35B MoE),
        // so NVFP4 rows are dequantized one QK_NVFP4 block at a time into
        // tmp_f32 -- never writing past its QK_NVFP4 bounds.
        ...
        const int nblock_nv = ncols / QK_NVFP4;
        // Dedicated thread-local scratch for the Q8_0 repack (both the up and
        // gate halves).  A per-call/recursive thread_local_work_buffer() would
        // be reallocated by the recursive iqk_moe_fused_up_gate() below, so we
        // must not use it here.  Rows are laid out with the full Nx stride so
        // the recursive call can read them back via ir*q8_row_size.
        thread_local std::vector<char> nvfp4_q8_work;
        if (nvfp4_q8_work.size() < 2*(size_t)Nx*q8_row_size) {
            nvfp4_q8_work.resize(2*(size_t)Nx*q8_row_size);
        }
        char * up_buf   = nvfp4_q8_work.data();
        char * gate_buf = nvfp4_q8_work.data() + (size_t)Nx*q8_row_size;
        auto repack_q8 = [&](const void * src, char * dst) {
            for (long ir = ith; ir < Nx; ir += nth) {
                const block_nvfp4 * row = (const block_nvfp4 *)((const char *)src + ir*strideA);
                for (int ib = 0; ib < nblock_nv; ++ib) {
                    // Dequant one QK_NVFP4 block to F32
                    dequantize_row_nvfp4(row + ib, tmp_f32, QK_NVFP4);
                    // Quantize that block to Q8_0
                    for (int jj = 0; jj < QK_NVFP4/qk8; ++jj) {
                        const int b = ib*(QK_NVFP4/qk8) + jj;
                        const int offset = jj*qk8;
                        float amax = 0.0f;
                        for (int j = 0; j < qk8; ++j) {
                            const float a = fabsf(tmp_f32[offset + j]);
                            if (a > amax) amax = a;
                        }
                        float d = amax / 127.0f;
                        if (amax == 0.0f) d = 1.0f;
                        const ggml_half d_half = GGML_FP32_TO_FP16(d);
                        char * dst_b = dst + ir*q8_row_size + b*(sizeof(ggml_half) + qk8);
                        memcpy(dst_b, &d_half, sizeof(ggml_half));
                        int8_t * qs = (int8_t *)(dst_b + sizeof(ggml_half));
                        const float inv_d = 1.0f / d;
                        for (int j = 0; j < qk8; ++j) {
                            const float v = tmp_f32[offset + j] * inv_d;
        ...
        repack_q8(Aup,   up_buf);
        repack_q8(Agate, gate_buf);
        ...
        if (!iqk_moe_fused_up_gate(Nx, Ny, ne00, ne11, unary_op,
                GGML_TYPE_Q8_0, up_buf, gate_buf, q8_stride,
                typeB, B, strideB,
                up_b_c, gate_b_c,
...
void iqk_fused_delta_net_impl(int n_heads, int gqa_ratio, int repeat_type, int n_tokens, int n_seqs,
    ...
    for (int h_idx = h_start; h_idx < h_end; ++h_idx) {
        ...
        if (ith == 0 && head_idx == 0) {
            float sn = 0.0f;
            for (int i = 0; i < head_dim*head_dim; ++i) sn += state_in[state_head_offset+i]*state_in[state_head_offset+i];
        }
        ...
        for (int t = 0; t < n_tokens; ++t) {
            ...
            if (ith == 0 && head_idx == 0) {
                float oval0 = _mm512_cvtss_f32(out_val[0]);
                float vnew0 = _mm512_cvtss_f32(v_prime[0]);
                float ot0   = _mm512_cvtss_f32(_mm512_loadu_ps(out_t));
                float vt0   = _mm512_cvtss_f32(_mm512_loadu_ps(v_t));
                float qt0   = _mm512_cvtss_f32(_mm512_loadu_ps(q_t));
                float kt0   = _mm512_cvtss_f32(_mm512_loadu_ps(k_t));
            }
            ...
            if (ith == 0 && head_idx == 0) {
                float sn = 0.0f;
                for (int i = 0; i < head_dim*head_dim; ++i) sn += state[i]*state[i];
            }
            if (ith == 0 && head_idx == 0) {
            }
            if (saved_steps && t + 1 < n_tokens) {
