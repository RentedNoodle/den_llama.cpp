...
struct FlashQKV {
    ...
    inline void normalize_and_store_1row(const FMS& fms, int j, qkv_cache_t * R, float * qkv, const float * sinkf) const {
        ...
        // S can be 0 if all keys for this query row are masked out (e.g. fully-causal mask on the
        // first token) or if the K*Q scores are NaN (garbage activations). Guard against a division
        // by zero instead of asserting; a zeroed output row is the safe fallback.
        float norm_val = S > 0.0f ? 1.0f/S : 0.0f;
        auto norm = F16::set1(norm_val);
