...
extern "C" IQK_API bool iqk_flash_attn_noalibi(int type_q, int type_mask, float max_bias,
    ...
    if (auto type_k = ggml_type(int_type_k_in), type_v = ggml_type(int_type_v); !are_kv_types_supported(type_k, type_v)) {
        if (ith == 0) {
            fprintf(stderr, "\n==================== K cache %s coupled with V cache %s is not a supported combination in the IQK flash attention path.\n",
                    ggml_type_name(type_k), ggml_type_name(type_v));
            fprintf(stderr, "    Falling through to the standard attention implementation.\n");
        }
        return false;
    }
