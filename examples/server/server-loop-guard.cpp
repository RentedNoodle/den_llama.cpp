...
server_loop_guard_result server_loop_guard::check(server_loop_guard_region region) const {
    ...
    if (periodic.triggered) return periodic;
    if (region == SERVER_LOOP_REGION_REASONING) {
        auto ngram = check_ngram_dominance(tokens);
        if (ngram.triggered) return ngram;
    }
...
server_loop_guard_result server_loop_guard::check_periodic_tail(const std::vector<llama_token> & tokens) const {
    ...
    for (int32_t p = 1; p <= max_period; ++p) {
        ...
        if (coverage < params.min_repeated_coverage) continue;
        ...
        int32_t mismatches = 0, matches = 0;
        for (int32_t i = p; i < n; ++i) {
            if (tokens[i] == tokens[i - p]) { matches++; }
            else if (++mismatches > max_mismatches) break;
        }
        if (mismatches <= max_mismatches) {
            return server_loop_guard_result { true, "periodic_tail", p, coverage, (float) matches / (float) coverage };
        }
        const float score = (float) matches / (float) coverage;
        if (score > best.score) {
            best = server_loop_guard_result { false, "periodic_tail", p, coverage, score };
        }
    }
    return {};
}
...
static bool server_loop_guard_equal_ngram(const std::vector<llama_token> & tokens, int a, int b, int ngram) {
    for (int i = 0; i < ngram; ++i) {
        if (tokens[a + i] != tokens[b + i]) return false;
    }
    return true;
}
...
server_loop_guard_result server_loop_guard::check_ngram_dominance(const std::vector<llama_token> & tokens) const {
    const int32_t n = (int32_t) tokens.size();
    if (n < params.min_repeated_coverage) return {};
    const int ngrams[] = {8, 16, 32};
    for (int ngram : ngrams) {
        if (n < ngram * 2) continue;
        ...
        for (size_t i = 1; i < intervals.size(); ++i) {
            if (intervals[i].first <= cur_end) cur_end = std::max(cur_end, intervals[i].second);
            else { coverage += cur_end - cur_begin; cur_begin = intervals[i].first; cur_end = intervals[i].second; }
        }
        coverage += cur_end - cur_begin;
        if (coverage >= params.min_repeated_coverage && coverage * 2 >= n) {
            return server_loop_guard_result { true, "ngram_dominance", ngram, coverage, (float) coverage / (float) n };
        }
    }
    return {};
}
