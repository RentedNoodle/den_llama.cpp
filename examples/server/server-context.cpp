...
const common_chat_msg& server_slot::update_chat_msg(bool is_partial, std::vector<common_chat_msg_diff>& diffs,
    ...
    auto new_msg = common_chat_parse(
        // Only parse the already-sent portion. While a multi-token stop string is
        // being buffered (partial stop match), generated_text holds trailing tokens
        // that are not yet sent and may later be erased; parsing them would leak
        // the stop word (e.g. "<|im_end|>") into the streamed OAI chat deltas.
        generated_text.substr(0, std::min(generated_text.size(), n_sent_text)),
...
bool server_context::launch_slot_with_task(server_slot& slot, server_task& task) {
    ...
    {
        // Qwen3.5-family chat templates end every turn with "<|im_end|>", but some
        // GGUF tokenizers fail to register it as a single token (it tokenizes into
        // several subword pieces), so it never lands in the EOG set and generation
        // never stops: the model keeps emitting "<|im_start|>user ..." turns forever
        // (chat completion loops). If the vocab has no single-token EOT and the chat
        // template uses "<|im_end|>", stop on the string so chat turns terminate
        // cleanly. The client-provided "stop" array above is kept; "<|im_end|>" is
        // added as an additional stop since it is the model's real end-of-turn.
        const llama_model * model_ctx = llama_get_model(slot.ctx);
        if (llama_token_eot(model_ctx) == LLAMA_TOKEN_NULL) {
            const int32_t n_tmpl = llama_model_meta_val_str(model_ctx, "tokenizer.chat_template", nullptr, 0);
            if (n_tmpl > 0) {
                std::string tmpl;
                tmpl.resize(n_tmpl);
                llama_model_meta_val_str(model_ctx, "tokenizer.chat_template", tmpl.data(), tmpl.size() + 1);
                if (tmpl.find("<|im_end|>") != std::string::npos) {
                    bool have_im_end = false;
                    for (const auto & ap : slot.params.antiprompt) {
                        if (ap == "<|im_end|>") {
                            have_im_end = true;
                            break;
                        }
                    ...
                    if (!have_im_end) {
                        slot.params.antiprompt.push_back("<|im_end|>");
                        LOG_WARNING("%s\n", "vocab has no single-token <|im_end|>; added it as a default stop string");
                    }
                }
            }
        }
