...
int main(int argc, char ** argv) {
    ...
    while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
        ...
        if ((int) embd_inp.size() <= n_consumed && !is_interacting) {
            ...
            // CATS batch-drafts K tokens in one forward pass, which evolves the
            // recurrent SSM state (kv_self.s_l) through all draft tokens and cannot
            // roll it back on rejection. Disable for recurrent/hybrid models.
